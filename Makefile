...
test-console-served-state-e2e: ## Console turn proved on the BROWSER-served surface (live frames + settlement)
	@uv run --project server python scripts/qa/console-served-state-e2e.py $(ARGS)
...
generate-managed-identity: ## Regenerate the engine managed-identity vocabulary from the provider schema
	@cd server && uv run python ../scripts/generate/managed_identity_contract_rs.py

generate-completeness: ## Regenerate the completeness inventory
	@cd server && uv run python ../scripts/generate_completeness_inventory.py

validate-qa-scripts: ## @internal Every scripts/qa check is reachable or declares why it is not
	@cd server && uv run python ../scripts/qa/check-qa-scripts-are-read.py
...
validate-managed-identity: ## @internal Guard the generated managed-identity vocabulary and its launch sites
	...
	@cd server && uv run python ../scripts/generate_completeness_inventory.py --check
