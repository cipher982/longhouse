...
hot-control-check: ## Binary PASS/FAIL for hot-control live receipt health
	@cd server && uv run python ../scripts/ops/hot-control-check.py --since "$${SINCE:-24h}"

hot-control-remind: ## Print/register the 24h hot-control dogfood check
	@echo "Run this after normal app use: make hot-control-check SINCE=24h"
	@osascript -e 'tell application "Reminders" to make new reminder with properties {name:"Run Longhouse hot-control check", body:"cd $(CURDIR) && make hot-control-check SINCE=24h", remind me date:(current date) + 24 * hours}' >/dev/null 2>&1 || true
