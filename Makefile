...
capture-live-status-frames: ## Capture replay states and motion (CAPTURE=<private.json>; optional SURFACE)
...
test-terminal-fidelity-gate: ## Compose real Console, web, iOS and recovery proofs (ARGS=explicit targets)
	@uv run --project server python scripts/qa/terminal-fidelity-gate.py $(ARGS)
...
ci-queue: ## How busy CI is: runner pools, queue, superseded main commits (SHA= to locate a commit)
	@./scripts/ops/ci-queue.py $(if $(SHA),--sha $(SHA),)
