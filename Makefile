...
test-session-state: ## @internal Focused canonical session-state and Phase 7 fault proofs
	@NODEIDS="$$(uv run --no-project --with pyyaml python scripts/qa/session-state-fault-matrix.py --nodeids --server-relative)"; \
		cd server && uv run --extra dev pytest -q $$NODEIDS
	@python3 scripts/tests/session-state-deep-health.test.py
...
test-cursor-console-product-e2e: ## Hosted Runtime Host -> Machine Agent -> stock Cursor Console proof
	@uv run --project server python scripts/qa/opencode-console-product-e2e.py --provider cursor $(ARGS)

test-cursor-helm-gate0: ## Real stock-Cursor Helm identity/hook proof (release-gated)
	@uv run --project server python scripts/qa/cursor-helm-gate0.py $(ARGS)

cursor-observed-install-qualification: ## Exact installed Cursor full-column snapshot (requires explicit paths/version)
	@test -n "$(CURSOR_BIN)" || (echo "CURSOR_BIN is required" >&2; exit 2)
	@test -n "$(CURSOR_ROOT)" || (echo "CURSOR_ROOT is required" >&2; exit 2)
	@test -n "$(CURSOR_GATE0_ARTIFACT)" || (echo "CURSOR_GATE0_ARTIFACT is required" >&2; exit 2)
	@test -n "$(CURSOR_VERSION)" || (echo "CURSOR_VERSION is required" >&2; exit 2)
	@test -n "$(OUTPUT_ROOT)" || (echo "OUTPUT_ROOT is required" >&2; exit 2)
	@uv run --project server python scripts/qa/cursor-observed-install-qualification.py \
		--provider-bin "$(CURSOR_BIN)" \
		--provider-root "$(CURSOR_ROOT)" \
		--gate0-artifact "$(CURSOR_GATE0_ARTIFACT)" \
		--expected-version "$(CURSOR_VERSION)" \
...
validate-perf-proof: ## Validate perf proof collector parsing and summary logic
	@python3 scripts/tests/perf-proof.test.py
...
qualification-native-binaries: ## @internal Build the exact native pair used by qualification targets
	@if [ "$(QUALIFICATION_BUILD_NATIVE)" = "1" ]; then \
		python3 scripts/build/generate_build_identity.py >/dev/null; \
		cargo build --manifest-path engine/Cargo.toml --profile ci --bin longhouse --bin longhouse-engine; \
	fi

test-installed-managed-launch-faults: | qualification-native-binaries ## @internal Native provider degraded launch + Runtime Host recovery qualification
	@uv run --project server python scripts/qa/installed-managed-launch-fault-matrix.py --longhouse-bin "$(QUALIFICATION_LONGHOUSE_BIN)" --engine-bin "$(QUALIFICATION_ENGINE_BIN)" --concurrent --cold-restart $(ARGS)

test-installed-native-health-faults: | qualification-native-binaries ## @internal Native local-health fault qualification
	@uv run --project server python scripts/qa/installed-health-fault-matrix.py --binary "$(QUALIFICATION_LONGHOUSE_BIN)" $(ARGS)
...
validate-session-state-deep-health: ## @internal Guard Phase 7 deep-health verdicts
	@python3 scripts/tests/session-state-deep-health.test.py

validate-no-python-device-path: ## @internal Inventory Python still on the device provider-control path
	@python3 scripts/qa/check-no-python-device-path.py
...
validate-tools: ## @internal Tool contract drift check
	@$(MAKE) generate-tools >/dev/null
	@if ! git diff --quiet -- server/zerg/tools/generated/tool_definitions.py; then \
		echo "Tool contracts out of sync — run 'make generate-tools'"; \
		exit 1; \
	fi
...
generate-tools: ## Regenerate Python tool contract types
	@cd server && uv run python ../scripts/generate_tool_types.py
