...
- **Stop hook calls `longhouse-engine ship --file "$TRANSCRIPT"` directly** (not via Python wrapper). Absolute path baked at install time via `get_engine_executable()`. `exec` replaces the shell — zero Python overhead. `async: true` to avoid blocking Claude between responses.
