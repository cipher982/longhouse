...
def test_release_proof_can_attach_universal_old_new_release_diff() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        root = Path(temp_dir)
        _write_fake_repo(root / "repo")
        _write_fake_provider_bin(root, "opencode 9.9.9")
        old = _write_release_proof(root, "old")
        new = _write_release_proof(root, "new", version="1.2.4")
        ...
        assert (
            payload["operation_evidence"]["universal_old_new_release_diff"]["level"]
            == "artifact_diff"
        )
        ...
        old_new = [
            ...
            if item["scenario"] == "old_new_release_diff"
        ][0]
        assert old_new["status"] == "pass"
        assert Path(old_new["data"]["old_new_diff_artifact_path"]).is_file()
...
def test_claude_release_proof_can_attach_universal_steer_active_turn_e2e() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        root = Path(temp_dir)
        repo = root / "repo"
        _write_fake_repo(repo)
        ...
        assert payload["verdict"] == "green"
        assert (
            payload["normalized"]["canaries"]["universal_steer_active_turn"]["status"]
            == "pass"
        )
        assert (
            payload["operation_evidence"]["universal_steer_active_turn"]["status"]
        ...
        assert (
            payload["operation_evidence"]["universal_steer_active_turn"]["level"]
            == "live_no_token"
        )
        assert payload["operation_evidence"]["universal_db_ingest"]["status"] == "pass"
        ...
        result_row = universal_artifact["results"][0]
        assert result_row["provider"] == "claude"
        assert result_row["scenario"] == "steer_active_turn"
        assert result_row["status"] == "pass"
        evidence_root = Path(result_row["evidence_root"])
        raw_events = (evidence_root / "events" / "provider-raw-events.jsonl").read_text(
            encoding="utf-8"
...
def test_opencode_release_proof_can_attach_universal_interrupt_cancel_e2e() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        root = Path(temp_dir)
        _write_fake_repo(root / "repo")
        _write_fake_opencode_server_bin(root)
...
def main() -> int:
    tests = [
        ...
        test_opencode_release_proof_can_attach_universal_interrupt_cancel_e2e,
