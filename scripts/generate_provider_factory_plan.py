...
# The same facts rendered as the two human-readable tables that the private
# provider-factory-coherence spec embeds verbatim. Generated here, in the
# public repo that owns the model, so the private repo can assert its spec is
# not stale by plain containment instead of importing longhouse Python across
# the repo boundary (see control-plane
# tests/test_provider_factory_status_tables_match_spec.py).
TABLES_OUTPUT_PATH = ROOT / "docs" / "generated" / "provider_factory_status_tables.md"

sys.path.insert(0, str(ROOT / "server"))
sys.path.insert(0, str(ROOT / "server" / "scripts"))

from render_provider_factory_status import render_diagonal_status  # noqa: E402
from render_provider_factory_status import render_status_table  # noqa: E402
...
def render_tables() -> str:
    ...
    return (
        ...
        "## Status table\n"
        "\n"
        f"{render_status_table(facts)}\n"
        "\n"
        "## Staged-release diagonal\n"
        "\n"
        f"{render_diagonal_status(facts)}\n"
    )
...
def main() -> int:
    ...
    rendered_tables = render_tables()
    current_tables = TABLES_OUTPUT_PATH.read_text(encoding="utf-8") if TABLES_OUTPUT_PATH.exists() else ""

    if args.check:
        for path, want, have in (
            (OUTPUT_PATH, rendered, current),
            (TABLES_OUTPUT_PATH, rendered_tables, current_tables),
        ):
            if want != have:
                print(
                    f"{path} is out of date; run scripts/generate_provider_factory_plan.py --write",
                    file=sys.stderr,
                )
                return 1
        return 0
    ...
    if args.write:
        ...
        TABLES_OUTPUT_PATH.write_text(rendered_tables, encoding="utf-8")
        return 0
