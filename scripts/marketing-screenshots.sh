...
if [ "$1" = "clips" ]; then
    ...
    uv run --project server --with playwright --with pyyaml scripts/capture_marketing_clips.py \
        --base-url "$BASE_URL" ${2:+--name "$2"}
    exit 0
elif [ -n "$1" ]; then
    uv run --project server --with playwright --with pyyaml scripts/capture_marketing.py \
