...
const frontendBaseUrl =
  process.env.PLAYWRIGHT_BASE_URL || process.env.E2E_FRONTEND_URL;
const apiBaseUrl =
  process.env.PLAYWRIGHT_API_BASE_URL || process.env.E2E_API_URL;
if (!frontendBaseUrl || !apiBaseUrl) {
  throw new Error(
    "playwright.prod.config.js requires explicit PLAYWRIGHT_BASE_URL and PLAYWRIGHT_API_BASE_URL targets",
  );
}
const journeyPrivacyMode = process.env.LONGHOUSE_JOURNEY_PRIVACY_MODE === "1";
const journeyRawOutputDir =
  process.env.LONGHOUSE_JOURNEY_RAW_OUTPUT_DIR || "test-results";
