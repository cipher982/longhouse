...
base.describe('Schema Isolation Guardrail', () => {
  ...
  base('data in commis A is NOT visible in commis B', async () => {
    ...
    expect(schemaA.db_path, `Commis A db debug: ${JSON.stringify(schemaA)}`).toBeTruthy();
    expect(schemaB.db_path, `Commis B db debug: ${JSON.stringify(schemaB)}`).toBeTruthy();
    expect(schemaA.db_path, `Expected different DBs but got same: A=${JSON.stringify(schemaA)} B=${JSON.stringify(schemaB)}`).not.toBe(
      schemaB.db_path
    );
    expect(schemaA.commis_id).toBe(COMMIS_ID_A);
    expect(schemaB.commis_id).toBe(COMMIS_ID_B);

    // Commis A: Create an automation with a unique marker in system_instructions.
    ...
    // Commis B: Create an automation with a different unique marker.
    ...
    // Commis A: List automations - should see automation A's marker, not B's.
    ...
    // Commis B: List automations - should see automation B's marker, not A's.
    ...
    console.log(`✅ Schema isolation verified: Commis A and B have separate data`);
  });
});
