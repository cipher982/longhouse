/**
 * Oikos non-blocking commis UX regression test.
 *
 * Ensures chat input re-enables after oikos_complete even if
...
test('chat input unblocks after oikos_complete while commis runs', async ({ page }) => {
  ...
  // Patch fetch so /api/oikos/chat never resolves (simulates long-running SSE stream).
  await page.evaluate(() => {
    ...
    window.fetch = (input: RequestInfo | URL, init?: RequestInit) => {
      const url = typeof input === 'string'
        ...
        : input instanceof URL
          ? input.toString()
          : input.url;
      if (url.includes('/api/oikos/chat')) {
