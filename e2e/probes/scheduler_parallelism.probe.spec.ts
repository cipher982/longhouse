...
test.describe("Scheduler Parallelism Probe", () => {
  test("probe config sanity", async ({}, testInfo) => {
    ...
    writeEvent(testInfo.parallelIndex, { type: "start", name: testInfo.title, workerIndex: testInfo.parallelIndex, t: Date.now() });
    await new Promise(resolve => setTimeout(resolve, 25));
    writeEvent(testInfo.parallelIndex, { type: "end", name: testInfo.title, workerIndex: testInfo.parallelIndex, t: Date.now() });
  });
