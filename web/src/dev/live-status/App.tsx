...
export default function App() {
  ...
  const reduceMotion = prefersReducedMotion || motionOverride === true;
  ...
  const omittedEntries = useMemo(() => {
    if (!capture) return 0;
    const capturedEntries = capture.workspace.projection.items.filter(
      (item) => item.kind !== "seam",
    ).length;
  ...
  const sessionTitle = session
    ? getSessionCardText(session, { titleMaxChars: 160 }).title
    : "";
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      const elapsed = Math.min(
        REPLAY_DURATION_MS,
        startedAtMs + now - startedAt,
      );
      const next =
        elapsed >= REPLAY_DURATION_MS
          ? REPLAY_DURATION_MS
          : Math.max(startedAtMs, Math.floor(elapsed / 50) * 50);
  ...
  async function loadCapture(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setFileError(
        "Could not read this capture. Choose a Longhouse live-status capture JSON file.",
      );
    } finally {
      ...
      <div className="lab-studio">
          ...
          >
            ...
            <div className="lab-controls__body">
              ...
              <section className="lab-control-section">
                ...
                <p
                  className="lab-capture-name"
                  title={captureName || undefined}
                >
                ...
                {fileError ? (
                  <p className="lab-file-error" role="alert">
                    {fileError}
                  </p>
                ) : null}
              </section>
              ...
              <section className="lab-control-section">
                <label className="lab-control-label" htmlFor="lab-scenario">
                  Scenario
                </label>
                <select
                  id="lab-scenario"
                ...
                <div className="lab-playback">
                  ...
                  <output className="lab-time" htmlFor="lab-time-range">
                    {formatReplayTime(timeMs)} /{" "}
                    {formatReplayTime(REPLAY_DURATION_MS)}
                  </output>
                </div>
                <input
                  id="lab-time-range"
                  ...
                  <input
                    ...
                    title={
                      prefersReducedMotion
                        ? "Your system Reduce Motion preference is enabled"
                        : undefined
                    }
                    onChange={(event) =>
                      setMotionOverride(event.target.checked)
                    }
                  />
              ...
              <p className="lab-controls__footnote">
                Capture and draft stay in this tab’s memory. Nothing is uploaded
                or sent. Transcript links and external media are disabled.
              </p>
              <p className="lab-controls__footnote lab-compact-note">
                The phone-sized view is a compact web design study, not a native
                iOS feature.
              </p>
            </div>
          </details>
        ...
        <main className="lab-stage">
          <div className="lab-stage__caption">
            <span>
              {capture ? provenance : "Live confidence · a design study"}
            </span>
          ...
          {capture && frame && timeline && session ? (
            ...
            <section className="lab-empty" aria-labelledby="lab-empty-title">
              <div className="lab-empty__content">
                <p className="lab-empty__eyebrow">
                  Real content. Considered confidence.
                </p>
                <h1 id="lab-empty-title">Start with a real session.</h1>
                <p className="lab-empty__intro">
                  Open a recorded Longhouse capture to explore how work,
                  silence, and connection changes should feel.
                </p>
                <button
                ...
                <div className="lab-empty__steps">
                  <div>
                    <span>01</span>
                    <p>
                      Choose a local capture.
                      <small>The original conversation stays intact.</small>
                    </p>
                  </div>
                  <div>
                    <span>02</span>
                    <p>
                      Explore a simulated state.
                      <small>Seek, pause, and compare the quiet details.</small>
                    </p>
                  </div>
                  <div>
                    <span>03</span>
                    <p>
                      Keep the real session untouched.
                      <small>No live connection. No commands sent.</small>
                    </p>
                  </div>
                </div>
                <p className="lab-empty__privacy">
                  Read locally, kept only in memory. Reload to clear.
                </p>
              </div>
            </section>
          )}
