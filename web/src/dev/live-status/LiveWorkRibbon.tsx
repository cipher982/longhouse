...
function ageText(seconds: number): string {
  ...
  if (age < 86_400) return `${Math.floor(age / 3_600)}h ${Math.floor((age % 3_600) / 60)}m ago`;
...
export function LiveWorkRibbon({ state, motionTimeMs, reduceMotion }: LiveWorkRibbonProps) {
  const workIsMoving = state.tone === "working" && state.animateWork && !reduceMotion;
  const workAngle = ((motionTimeMs % WORK_ROTATION_MS) / WORK_ROTATION_MS) * 360;
  const heartbeatGlint = !reduceMotion && state.connection === "connected" && state.heartbeatAgeMs !== null
    ? Math.max(0, 1 - Math.max(0, state.heartbeatAgeMs) / HEARTBEAT_GLINT_MS)
    : 0;
  const outputAge = state.outputAgeSeconds === null
    ? null
    : `Output received ${ageText(state.outputAgeSeconds)}`;
  const heartbeatDescription = state.heartbeatAgeMs === null
    ...
    >
      <div className="lwr-primary">
        <span
          ...
          style={workIsMoving ? { transform: `rotate(${workAngle}deg)` } : undefined}
        >
          <WorkGlyph tone={state.tone} />
        </span>
        <span className="lwr-headline" role="status" aria-live="polite" aria-atomic="true">
          {state.headline}
        </span>
        <span className="lwr-receipt-trail" data-testid="receipt-trail" aria-hidden="true">
          {state.receiptMarks.map((mark) => {
            if (mark.ageMs < 0 || mark.ageMs >= RECEIPT_WINDOW_MS) return null;
            const ageMs = reduceMotion ? Math.floor(mark.ageMs / 1_000) * 1_000 : mark.ageMs;
            const freshness = 1 - ageMs / RECEIPT_WINDOW_MS;
            return (
              <span
                key={mark.id}
                className="lwr-receipt-mark"
                data-replay={mark.replay ? "true" : "false"}
                style={{ right: `${(1 - freshness) * 85}px`, opacity: freshness }}
              />
            );
          })}
        </span>
      ...
      {state.detail && (
        <div className="lwr-command" title={state.detail}>
          {state.detail}
        </div>
      )}

      <details className="lwr-observation">
        <summary className="lwr-observation-toggle" title="Inspect the observed session evidence">
          <span
          ...
          <svg className="lwr-disclosure" viewBox="0 0 16 16" fill="none" aria-hidden="true" focusable="false">
            <path d="m5.5 6.5 2.5 2.5 2.5-2.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </summary>
        <div className="lwr-evidence">
          ...
          <p className="lwr-receipt-key">
            Receipt marks show content received over the last 12 seconds, not work progress. Hollow marks are replayed content.
          </p>
        </div>
      </details>
    </div>
