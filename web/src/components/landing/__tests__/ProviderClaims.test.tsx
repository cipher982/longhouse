...
describe("landing provider claims", () => {
  it("shows differentiated provider cards instead of parity copy", () => {
    ...
    expect(screen.getByText("Import, search, and direct cloud branching")).toBeInTheDocument();
    expect(screen.getByText("Import, search, and control-ready launches through Longhouse")).toBeInTheDocument();
    expect(screen.getByText("Import and search today; direct cloud branching later")).toBeInTheDocument();
  ...
  it("explains that direct cloud branching is still Claude-first", async () => {
    ...
    expect(
      screen.getByText(/Claude Code currently has the strongest cloud-branch path, hooks, and telemetry/i),
    ).toBeInTheDocument();
    expect(
