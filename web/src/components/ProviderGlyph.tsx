...
export function providerBrandColor(provider: string | null | undefined): string {
  ...
  return PROVIDER_BRAND[canonicalProvider(provider).toLowerCase()] ?? "#9A8F7E";
}

function normalize(provider: string | null | undefined): string {
  return canonicalProvider(provider ?? "").toLowerCase();
}
...
export function ProviderGlyph({
  ...
  const markColor = tone === "mono" ? "var(--color-text-secondary)" : brand;
  ...
  const tintBase = tone === "mono" ? "154, 143, 126" : hexToRgb(brand);
  return (
    <span
      className={className}
      style={{
