...
export class TimelineLogger {
  ...
  private setupListeners(): void {
    ...
    // Oikos lifecycle events
    this.unsubscribers.push(
      eventBus.on('oikos:started', (data) => {
        this.recordEvent('oikos_started', data.timestamp, { runId: data.runId, task: data.task });
      })
    );

    this.unsubscribers.push(
      eventBus.on('oikos:thinking', (data) => {
        this.recordEvent('oikos_thinking', data.timestamp, { message: data.message });
      })
    );

    this.unsubscribers.push(
      eventBus.on('oikos:complete', (data) => {
        this.recordEvent('oikos_complete', data.timestamp, {
          runId: data.runId,
          status: data.status,
    ...
    this.unsubscribers.push(
      eventBus.on('oikos:error', (data) => {
        this.recordEvent('oikos_error', data.timestamp, { message: data.message });
        this.outputTimeline();
      })
    );
    ...
    this.unsubscribers.push(
      eventBus.on('oikos:deferred', (data) => {
        this.recordEvent('oikos_deferred', data.timestamp, { runId: data.runId, message: data.message });
        this.outputTimeline();
      })
    );
  }
