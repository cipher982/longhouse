...
- [x] Publish a newer runner release for canary verification
- [x] Verify one real canary self-updates under policy `apply`
...
- [x] Remove user-facing `commis` / `autonomous agents` / server-first wording from prompts, product copy, and operator pages
- [x] Publish one honest provider capability story for archive, cloud session start, continuation, hooks, and telemetry
- [x] Rename launch-facing cloud work labels from `commis` to `cloud session` or equivalent while keeping internal symbol churn minimal
- [x] Remove fiche/dashboard-era surfaces from default onboarding and primary navigation unless they are directly required
...
- 2026-03-17: Shipped the Playwright selector/API cleanup. Dashboard/browser tests now use `create-automation-btn`, `data-automation-id`, and the canonical `/api/automations` surface; the dead dashboard search spec was deleted, and the targeted dashboard/chat/realtime Playwright batches passed after fixing two stale readiness assumptions plus one brittle thread-row assertion.
- 2026-03-17: Shipped the remaining E2E/helper naming cleanup. The Playwright helper module is now `automation-helpers.ts`, helper types/logs/spec titles use automation-first language, and the renamed spec files (`automation_creation*.spec.ts`, `core/automation-crud.spec.ts`) are fully wired without touching backend `fiche_id` storage or the stable `/fiche/:id/thread` route.
- 2026-03-17: Verification for the E2E/helper naming cleanup passed via `git diff --check`, `make test-e2e-single TEST='--project=chromium tests/automation_creation.spec.ts tests/automation_creation_full.spec.ts tests/core/automation-crud.spec.ts tests/realtime_updates.spec.ts tests/realtime_websocket_monitoring.spec.ts tests/chat_token_streaming.spec.ts'`, and `make test-e2e-single TEST='--project=chromium tests/happy-paths.spec.ts tests/commis_isolation.spec.ts tests/commis_isolation_guardrail.spec.ts tests/automation_history.spec.ts tests/run_optimistic_update.spec.ts tests/comprehensive_database_isolation.spec.ts tests/error_handling_edge_cases.spec.ts tests/core/chat-send.spec.ts tests/core/data-persistence.spec.ts tests/core/thread-management.spec.ts tests/dashboard.scope-toggle.spec.ts tests/core/session-continuity.spec.ts'`.
- 2026-03-17: Shipped the remaining frontend/browser automation naming cleanup outside the dashboard/Oikos seams. Chat hooks/pages now prefer `automationId`, settings/contacts/integrations/runners/knowledge copy is automation-first, and the lingering connector/contact comments/docs no longer teach `fiche` as the browser concept.
...
- 2026-03-17: Current slice: rename the dashboard's canonical frontend overview shape (`fiches` / `ficheId`) to `automations` / `automationId`, then clean the still-live Oikos `/fiches` frontend/API surface into task terminology while keeping backend/storage ownership stable.
...
- [x] Add the thinnest possible Oikos-owned state for trigger history / policies without duplicating session logs
...
- 2026-03-11: Spec recap landed in `docs/specs/oikos-proactive-operator.md` and `docs/plans/oikos-autonomy-roadmap.md`. The current dogfood shape is now explicit: runtime wakeups are `presence.blocked`, `presence.needs_user`, `periodic_sweep`, and recent post-ingest `session_completed`.
- 2026-03-11: The first wakeup-ledger slice landed as `oikos_wakeups`. Presence, post-ingest completion, and periodic sweep now persist `suppressed`, `enqueued`, and enqueue-time `failed` outcomes with compact payloads plus `run_id` when a run is created. Post-run `ignored` / `acted` classification is still the next step.
...
- [x] Fix capability registration so install paths can request explicit capabilities
- [x] Add shared runner health assessment and use it across API/tool/prompt surfaces
...
Status (2026-03-12): In progress (email reply MVP + inbox slice landed; surface migration still pending).
...
- 2026-03-12: Gmail connector ingress now persists inbound and replayed mailbox messages into conversations inside `GmailProvider.process_connector()`, with raw `.eml` archive coverage and duplicate replay protection.
...
- [x] Rename the live automation websocket/event contract off `fiche:*` / `fiche_updated` / `fiche_state`
- [x] Verify with SDK generation, frontend units, and `make test`
