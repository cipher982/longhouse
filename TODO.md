...
- [Docs vs UI] `longhouse auth` instructs users to open `/dashboard/settings/devices`, but there is no device-token UI or route; frontend only has `/settings` and no device token page. **Tracked as feature gap** — needs device-token management page at `/settings/devices`.
