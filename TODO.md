...
- 2026-03-17: Current slice: rename the remaining E2E/helper naming surface off `fiche` terms (`helpers/fiche-helpers.ts`, helper type names, and old `fiche_*` spec filenames) while keeping backend storage/query params and the `/fiche/:id/thread` chat route stable.
