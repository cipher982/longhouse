...
## Goal
Complete full French terminology renaming (no backward compatibility). Rename tables/columns, Python classes, API schemas, endpoints, tests, and frontend to Fiche/Run/Commis naming.
...
- 2026-01-26: Updated `main.py` router imports/includes + oikos endpoints to fiche/run naming. Adjusted MCP server routes to `/fiches/{fiche_id}/mcp-servers`.
- 2026-01-26: Renamed run history → run history, memory tools → fiche_memory, updated oikos/threads/connectors/websocket metrics to fiche/run terms. Cloud executor now uses run_commis.
...
- 2026-01-26: Backend models/services/routers/schemas updated to fiche/run/commis naming; ops/usage/reliability endpoints now return run/fiche metrics.
...
- 2026-01-26: Frontend API migration complete (fiches/runs/commis), Reliability UI updated, commis E2E headers standardized.
...
- 2026-01-26: Fixed MCP server payloads to include `transport` and dashboard websocket run `display_type` default; frontend typecheck + contract validation pass.
...
- [x] Models/tables/columns/relationships align with Fiche/Run/Commis
- [x] CRUD/services/routers updated (oikos + main entrypoints)
- [x] Schemas updated
- [x] Generated WS/SSE types regenerated
- [x] Alembic migration added for renames

### Phase C — Frontend & client contracts
- [x] API endpoints updated (/fiches, /runs)
