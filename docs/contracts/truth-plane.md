...
| Runtime display | What is happening right now, and how trustworthy is that signal? | `SessionRuntimeDisplayResponse` from `server/zerg/services/session_runtime_display.py` | runtime display tests and timeline status tests | Add cross-client fixtures that assert the exact label/tone/detail clients render. |
...
| Session input lifecycle | What state is a submitted user input in after send, retry, crash, or cancel? | `SessionInput.status` plus helpers in `server/zerg/services/session_inputs.py` | existing input service tests | Add idempotency, retry, attachment, and boot-recovery transition tests. |
| Host and transport health | Is the host reachable, is the control transport alive, and are those different? | runtime facts and runtime display overlays | existing reducer/runtime tests | Collapse duplicated host/transport wording into stable backend reason codes. |
| Remote launch lifecycle | Did a remote launch start, become live, fail, or become orphaned? | session launch fields and `SessionLaunchAttempt` projection | launch route tests | Add explicit transition matrix tests and client fixture coverage. |
| Provisional vs durable transcript | Is this text live preview, durable archive, stale preview, or superseded? | `SessionTranscriptPreview` and durable events | preview freshness tests | Add cross-client stale/superseded fixtures. |
