...
| Runtime display | What is happening right now, and how trustworthy is that signal? | `SessionRuntimeDisplayResponse` from `server/zerg/services/session_runtime_display.py` | runtime display tests plus web/iOS precedence tests that keep `runtime_facts` as compatibility/debug fallback | Add cross-client fixtures that assert the exact label/tone/detail clients render from shared payloads. |
