...
**Current grand-epic score:** 90/100
...
movement for this epic is release-watch proofing before/after: roughly 20 -> 90.
...
| Baselines and differential confidence | 15 | 13 | Accepted baseline machinery exists and now compares universal action/control artifacts plus provider execution-coverage artifacts through both the baseline CLI and universal harness, with structured action/coverage drift summaries for old/new results; staged-binary old/new runner can produce both sides, write a checksummed staging manifest, and diff them; default all-provider fake/no-token universal smoke is CI-gated, generates provider-scoped synthetic old/new proof pairs, and verifies `old_new_release_diff` coverage for every provider; private exact-version install/fetch source of truth is still Sauron-owned future work |
...
- `provider-release-proof-baseline.py` now emits structured `action_drift` and
  `execution_coverage_drift` summaries. These identify changed abstract
  actions, required-evidence buckets, execution coverage status/kind changes,
  rollup-only drift, and unavailable optional execution-coverage artifacts.
...
- `permission_prompt` is an executable universal scenario. OpenCode proves the
  bridge reply transport hermetically. Codex proves fake app-server approval
  request handling hermetically through `canary_auto_approves_server_requests`.
  Claude still reports a typed `blocked` live held-prompt canary gap.
  Antigravity reports a typed `unsupported_gap` until it exposes stable
...
| H57 | Remove rejected portable remote-launch coverage | Superseded | +0 | The `launch_remote` action and projection were deleted with `session.launch`; provider proof now covers terminal-originated Helm controls and turn-scoped Console execution. |
...
| H80 | Promote Codex permission prompt to hermetic fake app-server proof | Done | +0 | Universal Codex `permission_prompt` now calls the provider-release fake app-server canary, records `codex_fake_app_server_permission_approval` evidence, and reduces default smoke proof-gap cells from 5 to 4 while keeping live held-permission prompts as the next gate |
...
1. Add private/Sauron provider-version install staging that feeds
   `provider-release-proof-staged-old-new` with audited old/new binary paths and source URIs; Longhouse now records the resulting checksum manifest.
