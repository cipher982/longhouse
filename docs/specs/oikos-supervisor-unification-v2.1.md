# Oikos ↔ Supervisor Unification (v2.1)

**Date:** December 19, 2025
**Audience:** Implementation handoff (Oikos web + Zerg backend)
...
5. **Simplify codepaths**: one streaming mechanism in Oikos web for Supervisor output.
...
- Voice transcripts are routed to Supervisor (`apps/zerg/frontend-web/src/oikos/app/hooks/useOikosApp.ts` handles transcripts)
- Typed text is routed to Supervisor (`apps/zerg/frontend-web/src/oikos/app/hooks/useTextChannel.ts` uses `SupervisorChatController` → POST `/api/oikos/chat`)
...
- Reduce/remove any remaining reliance on `/api/oikos/supervisor/events` in the UI (keep it for debugging/integrations).
- Continue simplifying Realtime plumbing so it stays I/O-only (if/when Realtime is enabled).

### Supervisor streaming

- `/api/oikos/chat`: fetch + manual SSE parsing (`apps/zerg/frontend-web/src/oikos/lib/supervisor-chat-controller.ts`)
- `/api/oikos/chat` SSE now includes worker lifecycle + worker tool events, plus supervisor tool monitoring events (ToolCards UI).
- `/api/oikos/supervisor/events`: EventSource streaming (legacy/programmatic; kept for debugging/integrations)
...
1. Configure Realtime turn detection with **`create_response=false`** so the server does not automatically create a response at end-of-turn.
   - Code lives in `apps/zerg/frontend-web/src/oikos/lib/session-handler.ts` (`SESSION_CONFIG.turnDetection`).
   - With the current SDK, this maps to `turn_detection.create_response` under the hood.
2. Ensure Oikos web never calls APIs that create a Realtime response:
...
- Stop advertising it in `/api/oikos/bootstrap`.
- Remove it from Oikos prompts (both server bootstrap prompt + any client fallback context text).
- Delete or rewrite tests that assert it exists.

Acceptance criteria:

- Bootstrap `enabled_tools` does not include `route_to_supervisor`.
- No codepath in Oikos web calls `/api/oikos/supervisor` as part of normal UX.
...
- Oikos web removes direct tool plumbing (no browser calling `/api/oikos/tool`).
...
### 5.1 Primary Oikos UI endpoint: `POST /api/oikos/chat`
...
### 5.2 Keep `/api/oikos/supervisor` + `/api/oikos/supervisor/events` as “programmatic”
...
But Oikos web should not depend on them for normal UX once `/chat` streams full progress.
...
We standardize tool names to match what Oikos UI expects to claim (keep human-friendly names):
...
## 7) Oikos Web UI Behavior (single output stream)
...
- ~~Remove `route_to_supervisor` tool from Oikos web~~ (DONE - legacy controllers deleted)
- Remove it from `/api/oikos/bootstrap` tool list + prompt text.
...
### Phase 3 — One Supervisor streaming path for Oikos web

**Outcome:** Oikos web needs only `/api/oikos/chat` for Supervisor output + progress.

Tasks:

- Extend `/api/oikos/chat` SSE generator to also subscribe to worker + worker tool events (parity with `/supervisor/events`).
- Update `SupervisorChatController` (`apps/zerg/frontend-web/src/oikos/lib/supervisor-chat-controller.ts`) to handle those events and publish `stateManager` updates for progress UI.
- Deprecate any Oikos web usage of `OikosAPIClient.executeSupervisorTask()` path.
...
1. Realtime enablement: keep always-on or behind a flag (recommended: `VITE_OIKOS_ENABLE_REALTIME=true/false`)?
...
- Oikos web uses a single Supervisor stream path (`/api/oikos/chat`) for UX.
