...
- 2026-03-08: The follow-up reconnect flake after re-enroll + reboot was hardened in `runner-v0.1.3`. The runner client now has an explicit websocket connect watchdog, so if the opening handshake stalls during boot it terminates the socket and schedules a reconnect instead of hanging indefinitely before `hello`.
...
- 2026-03-08: Implemented a disposable Linux VM canary with `scripts/runner-vm-canary.sh` and `scripts/runner-vm-canary-host.sh`. Live validation on `cube` now proves the full contract: Ubuntu `noble` cloud image -> `server` install -> guest reboot -> hosted runner online -> Oikos `runner_exec hostname -s` -> promote runner to `exec.full` -> re-enroll same VM -> guest reboot -> Oikos `runner_exec bash -lc 'hostname -s'` -> runner revoke -> VM destroy.
...
- Runner capability changes are dual-sided: updating the backend row to `exec.full` is not enough by itself because the runner process also validates commands locally from `RUNNER_CAPABILITIES`. The disposable canary therefore re-enrolls/restarts the VM after promotion before asking Oikos to run `bash -lc`.
