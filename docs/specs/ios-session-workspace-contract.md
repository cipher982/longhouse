...
Raw `/events` remains useful for future pagination, export/debug, or low-level
API compatibility, but it is not the session-detail bootstrap contract.
...
- Add or update backend tests for both agents and timeline routes hitting the
...
- Leave `sessionEvents` in `LonghouseAPI` only if a real caller remains or an
  explicit near-term pagination phase needs it.
...
- Implementing full projection pagination on iOS in this change.
