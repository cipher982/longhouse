# Oikos Long-Term Memory (Zerg-Native) Specification
...
Oikos currently relies on a single long-lived supervisor thread stored in the database. That thread grows without compaction, which eventually breaks recall and inflates prompt size. This spec defines a **Zerg-native long-term memory system** (no LangChain/Agent Builder dependencies) that preserves the best ideas from “memory files” while fitting the existing architecture.
