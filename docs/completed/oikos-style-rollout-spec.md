...
# Oikos Look-and-Feel → Swarmlet UI System (Spec + Roadmap)
...
## Why this exists
Oikos chat (`/chat`) has a cohesive, “premium” cyber/glass aesthetic (depth, glow, motion, typography). The rest of the Swarmlet SPA (dashboard + settings + runners + admin + canvas) is a mix of:
- legacy global CSS
- one-off page CSS
- inconsistent tokens / hardcoded colors

Goal: capture what makes Oikos feel “done”, distill it into a shared UI system, then roll that style across the rest of the app without copying Oikos CSS wholesale.

## Target outcome (plain English)
1) The whole app looks like one product (not “Oikos vs dashboard”).
2) The default aesthetic is “Oikos-ish”: glassy surfaces, crisp typography, subtle glow + motion.
...
**Oikos styling + theme**
- `apps/zerg/frontend-web/src/oikos/styles/index.css`
- `apps/zerg/frontend-web/src/oikos/styles/base.css`
- `apps/zerg/frontend-web/src/oikos/styles/layout.css`
- `apps/zerg/frontend-web/src/oikos/styles/chat.css`
- `apps/zerg/frontend-web/src/oikos/styles/sidebar.css`
- `apps/zerg/frontend-web/src/oikos/styles/voice-button.css`
- `apps/zerg/frontend-web/src/oikos/styles/animations.css`
- `apps/zerg/frontend-web/src/styles/tokens.css` (Oikos “glass” theme via `.oikos-container`)
...
- `apps/zerg/frontend-web/src/styles/css/nav.css` already borrows Oikos-ish header/nav styling.
...
- Oikos is already scoped under `.oikos-container`; keep that boundary until primitives are extracted.

## Key findings (what makes Oikos feel “premium”)
This is what we should systematize (not copy/paste).

### 1) Clear “surface language”
Oikos uses a consistent ladder of surfaces:
- void background
...
Oikos has intensive elements (voice reactor rings, animated grids). Those should be opt-in for specific pages/components, not the default for information-dense screens.
...
- Oikos: route-local CSS, aggressively scoped under `.oikos-container`
...
- animated grid layers (global `#react-root::before`, Oikos grid floor, nebula drift)
...
- Oikos background drift blobs: `apps/zerg/frontend-web/src/oikos/styles/layout.css` (`.app-container::after`, `nebula-drift`)
- Oikos grid floor: `apps/zerg/frontend-web/src/oikos/styles/layout.css` (`.main-content::before`, `grid-drift`)
- Oikos reactor rings: `apps/zerg/frontend-web/src/oikos/styles/voice-button.css` (ring animations)
...
- **Focus rings**: a single focus style (Oikos cyan default; brand primary for links).
...
Optional (Oikos-only or hero areas):
- animated grid floor
- nebula drift blobs
- reactor rings
...
- adopt Oikos surface language (glass card, borders, hover)

Goal: dashboard looks like “Oikos’s sibling”, not “a different app”.
...
**Dashboard:**
- `/dashboard` matches the Oikos surface language: glass cards, consistent borders, hover/focus style, consistent typography.
- Page CSS selectors are scoped under a page root class (no generic `.empty-state`, `.back-button`, etc. at top-level).

## Progress Summary
All phases of the Oikos Look-and-Feel rollout have been implemented. The app now uses a unified UI system based on shared primitives and design tokens. Heavy effects are gated behind the `VITE_UI_EFFECTS` environment variable.
...
- [x] Gate heavy FX behind `#react-root[data-ui-effects="on"]` in:
  ...
  - [x] `apps/zerg/frontend-web/src/oikos/styles/layout.css` (grid floor + nebula drift)
  - [x] `apps/zerg/frontend-web/src/oikos/styles/voice-button.css` (reactor animations)
...
- [x] Convert “Create Agent” to `Button --primary` (Oikos-style hover + subtle glow).
...
- We progressively migrate shared Oikos styling into global primitives; `/chat` keeps only chat-specific visuals (message bubbles, voice controls).
