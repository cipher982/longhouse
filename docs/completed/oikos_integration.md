...
# Oikos Integration Architecture

> **⚠️ DEPRECATED**: This document describes the original architecture.
> As of December 2024, oikos-server has been removed and all functionality moved to backend at `/api/oikos/*`.
...
The Swarm Platform integrates **Oikos** (voice/text UI) with **Zerg** (agent orchestration backend) to provide a unified AI assistant experience. Oikos serves as the human interface while Zerg handles all scheduling, workflows, and tool execution.
...
Authenticate Oikos device and establish a session.
...
1. Validates device secret against `OIKOS_DEVICE_SECRET` env var
2. Creates or fetches `oikos@swarm.local` ADMIN user
3. Issues JWT token (7-day expiry) and sets HttpOnly cookie (`oikos_session`)
4. Oikos stores expiry metadata locally for re-auth reminders

**Errors**:

- `401 Unauthorized`: Invalid device secret
- `500 Internal Server Error`: OIKOS_DEVICE_SECRET not configured

### Agent Management

#### GET /api/oikos/agents

List all available agents with their schedules.

**Authentication**:

- HttpOnly session cookie set during `/api/oikos/auth`
...
**Use Case**: Populate Oikos UI with available agents for voice/text commands like "Run my morning digest"
...
- HttpOnly session cookie set by `/api/oikos/auth`
- Development override: when `AUTH_DISABLED=1`, standard dev auth applies
...
**Use Case**: Display in Oikos Task Inbox, sorted by most recent first

### Task Dispatch

#### POST /api/oikos/dispatch

Trigger immediate agent execution from Oikos.
...
**Use Case**: User says "Check my health stats" → Oikos dispatches Health Watch agent

### Real-Time Events

#### GET /api/oikos/events
...
```
event: connected
data: {"message": "Oikos SSE stream connected"}
```
...
- Oikos opens SSE connection on startup
...
3. If no token OR expired:
   → POST /api/oikos/auth with OIKOS_DEVICE_SECRET
   → Store token in localStorage
   ↓
4. GET /api/oikos/agents
   → Cache agent list for voice/text dispatch
   ↓
5. GET /api/oikos/events
   → Open SSE connection for real-time updates
   ↓
6. GET /api/oikos/runs (limit=20)
...
User: "Run my morning digest"
   ↓
1. Oikos speech recognition → text
   ↓
2. Parse intent → agent_id lookup
   ↓
3. POST /api/oikos/dispatch { agent_id: 1 }
   ↓
...
4. Event bus publishes run_created
   ↓
5. Oikos SSE stream receives event
...
7. Event bus publishes run_updated (success, summary)
   ↓
8. Oikos SSE stream receives event
...
```typescript
{
  session_cookie_name: string; // "oikos_session"
...
- Oikos uses a pre-shared **device secret** stored in environment
- Secret is exchanged for a JWT token with 7-day expiry
- Token includes `oikos@swarm.local` user identity with ADMIN role
...
- All `/api/oikos/*` endpoints require the HttpOnly `oikos_session` cookie issued by `/api/oikos/auth`
- Cookie is validated via `get_current_oikos_user`
...
JARPXY_PORT=30080         # External: Oikos PWA entry point
ZGPXY_PORT=30081          # External: Zerg Dashboard entry point
OIKOS_WEB_PORT=8080      # Internal: oikos-web container
OIKOS_SERVER_PORT=8787   # Legacy: oikos-server (removed; backend now serves /api/oikos/*)
ZERG_BACKEND_PORT=8000    # Internal: backend container
ZERG_FRONTEND_PORT=5173   # Internal: frontend-web container

# Existing Zerg vars
OIKOS_DEVICE_SECRET="your-secure-secret-change-me-min-32-chars"
OPENAI_API_KEY="sk-..."
...
- Oikos PWA: http://localhost:30080
...
**Oikos Client** _(was `apps/zerg/frontend-web/src/oikos/.env`, now integrated at `apps/zerg/frontend-web/`)_:
...
# Note: Unified mode uses /api/oikos/session (served by backend; oikos-server removed)
...
COOKIE_JAR=cookies.txt
curl -s -X POST http://localhost:47300/api/oikos/auth \
  -H "Content-Type: application/json" \
  -d '{"device_secret":"your-secret"}' \
  -c "$COOKIE_JAR" -b "$COOKIE_JAR"

# List fiches
curl http://localhost:47300/api/oikos/fiches \
  -b "$COOKIE_JAR"

# Dispatch fiche
curl -X POST http://localhost:47300/api/oikos/dispatch \
  -H "Content-Type: application/json" \
  -d '{"fiche_id":1}' \
  -b "$COOKIE_JAR"

# Listen to SSE stream
curl -N http://localhost:47300/api/oikos/events \
  -b "$COOKIE_JAR"
```

**Note**: In unified mode, API routes are split between:

- Legacy: `/api/session`, `/api/tool`, `/api/sync/*` → oikos-server (Port 8787) (removed)
...
### Integrating in Oikos PWA

```typescript
import { getOikosClient } from "@oikos/core";

// Initialize client
const client = getOikosClient(import.meta.env.VITE_ZERG_API_URL);
...
ALTER TABLE runs ADD COLUMN summary TEXT NULL;
...
VALUES ('oikos@swarm.local', 'oikos', 'ADMIN', 'Oikos Assistant');
```
...
```python
{
    "event_type": "run_created" | "run_updated" | "fiche_updated",
    "run_id": int,
    "fiche_id": int,
    "status": str,
    "summary": str | None,
...
- **Connection limit**: 1 per Oikos instance
...
- **Agent list**: Cache for 60 seconds in Oikos
...
1. Check `OIKOS_DEVICE_SECRET` matches in both .env files
...
2. Check backend logs for "Oikos SSE stream connected"
...
- Check `/api/oikos/runs` for active run
...
2. Visual workflow builder in Oikos
...
- **Progress Report**: `/OIKOS_INTEGRATION_PROGRESS.md` _(archived/removed)_
- **Backend Router**: `/apps/zerg/backend/zerg/routers/oikos.py` _(still valid)_
- **TypeScript Client**: `/apps/zerg/frontend-web/src/oikos/core/oikos-api-client.ts` _(now: `apps/zerg/frontend-web/src/oikos/core/oikos-api-client.ts`)_
- **Seed Script**: `/apps/zerg/backend/scripts/seed_oikos_fiches.py` _(still valid)_
