...
# Oikos “Typing” Indicator Spec (Option C: Two-Step Confirm)
...
## Mapping to Current Oikos Events (Implementation Notes)

This section documents suggested wiring to existing Oikos web events as of December 2025.
...
> **Note**: Since this spec was written, Oikos was integrated into the Zerg frontend.
> Path references below refer to the old standalone structure.
> Current location: `apps/zerg/frontend-web/src/oikos/`

- **Supervisor lifecycle (delegation + run):** `apps/zerg/frontend-web/src/oikos/lib/event-bus.ts` _(now: `apps/zerg/frontend-web/src/oikos/lib/event-bus.ts`)_
...
- **Assistant streaming content:** `apps/zerg/frontend-web/src/oikos/lib/state-manager.ts` _(now: `apps/zerg/frontend-web/src/oikos/lib/state-manager.ts`)_
