...
# Oikos E2E Test Failure Investigation - Work Log
...
**What was attempted**: Fix failing E2E tests for text message flow in Oikos PWA
...
oikos-web:
  volumes:
    - ../../apps/zerg/frontend-web/src/oikos/src:/app/apps/web/src:ro
    - ../../apps/zerg/frontend-web/src/oikos/lib:/app/apps/web/lib:ro
    - ../../apps/zerg/frontend-web/src/oikos/packages:/app/packages:ro
    - ./tests:/app/tests:ro
```
...
```bash
make test-oikos-unit        # Fast unit tests (3 sec)
make test-oikos-watch       # TDD watch mode
make test-oikos-text        # Specific E2E test
make test-oikos-history     # Another specific test
make test-oikos-grep GREP="test name"  # Search by name
make test-oikos-e2e-ui      # Interactive UI
```

**Previous**: `make test-oikos` ran ALL 213 tests
**Now**: Run exactly what you need

### 3. Local Testing Script 🔍

**File**: `apps/zerg/frontend-web/src/oikos/test-local.sh`
...
**File**: `apps/zerg/frontend-web/src/oikos/TESTING.md`
...
```
M apps/zerg/frontend-web/src/oikos/src/context/types.ts
M apps/zerg/frontend-web/src/oikos/src/hooks/useRealtimeSession.ts
M apps/zerg/frontend-web/src/oikos/src/hooks/useTextChannel.ts
M apps/zerg/frontend-web/src/oikos/tests/text-channel-persistence.test.ts
```
...
```
M Makefile
M apps/zerg/frontend-web/src/oikos/docker-compose.test.yml
A apps/zerg/frontend-web/src/oikos/TESTING.md
A apps/zerg/frontend-web/src/oikos/test-local.sh
```
...
cd apps/oikos
docker compose -f docker-compose.test.yml up oikos-server oikos-web
...
```bash
make test-oikos-unit        # Verify unit tests still pass
make test-oikos-text        # Check if E2E tests now pass
```
...
```
apps/zerg/frontend-web/src/oikos/
├── src/
│   ├── components/
...
apps/zerg/frontend-web/src/oikos/tests/
...
├── oikos-server  (port 8787)  # Backend API
├── oikos-web     (port 8080)  # Vite dev server with HMR
...
make test-oikos-unit

# Specific E2E test with hot reload
make test-oikos-text
...
- **Testing Guide**: `apps/zerg/frontend-web/src/oikos/TESTING.md`
- **Test Script**: `apps/zerg/frontend-web/src/oikos/test-local.sh`
- **Make Targets**: Run `make help | grep oikos`
