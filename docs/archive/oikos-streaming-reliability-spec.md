# Oikos Real-Time Streaming Reliability

## Context + Post‑Mortem (What Happened)

Oikos delivers “durable runs” progress to the browser as a real-time event stream (heartbeats, tool start/complete, worker complete, supervisor complete). The system largely worked, but refresh/reconnect behavior felt inconsistent:
...
Nginx does not inherently understand SSE; it applies behaviors based on which `location` matches the URL. Some Oikos endpoints were explicitly configured as “streaming” (buffering off, long timeouts), but not all.
...
**Why it’s clean:**
- A single channel for all real-time (Oikos + other app signals).
