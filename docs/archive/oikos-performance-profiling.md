# Oikos Performance Profiling - Handoff Document
...
The goal is to build comprehensive E2E performance profiling for Oikos chat that measures **real user experience** - what users actually see and when they see it.
...
Modified `apps/zerg/frontend-web/src/oikos/lib/timeline-logger.ts` to output plain `console.log()` lines in addition to `console.groupCollapsed()`:
...
Occasionally see `Failed to clear Oikos history (worker=X): HTTP 500` in parallel tests. This doesn't break tests but indicates a race condition in the backend.
...
| `apps/zerg/frontend-web/src/oikos/lib/timeline-logger.ts` | Frontend timeline logger (modified) |
| `apps/zerg/frontend-web/src/oikos/app/components/ChatContainer.tsx` | Message DOM structure |
| `apps/zerg/frontend-web/src/oikos/app/components/WorkerProgress.tsx` | Worker progress panel |
