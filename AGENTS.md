...
11. **Runtime deploys and control-plane deploys are separate lanes** — runtime-path pushes build the GHCR runtime image, then deploy the public demo runtime from that image and reprovision the hosted canary. Superseded branch CI/image/deploy runs now cancel instead of burning time on stale branch commits. `control-plane/**` pushes deploy only the control plane. The control plane is still its own Coolify build.
