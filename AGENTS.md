...
- (2026-03-09) [continuation] `hatch -b zai` can create/resume Claude-compatible sessions, but its current `stream-json` path omits Claude's required `--verbose`. Keep the live `/api/sessions/{id}/chat` SSE route on direct `claude` invocation until hatch is fixed; use `SESSION_CHAT_BACKEND=zai` there for CI/provider-backed smoke instead.
