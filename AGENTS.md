...
- Warp-style CLI agent detection appears to key off the final spawned executable basename in the PTY. For managed Codex, `longhouse codex` should use the user's PATH `codex` first; debug the private `.../runtimes/codex/current/codex` payload only when the managed fallback runtime is actually selected.
...
- Managed Codex ships **stock upstream** `codex` (no fork). `scripts/release/download-managed-codex.sh` pulls openai/codex's release asset for the fallback runtime; normal `longhouse codex` launches should prefer the user's PATH `codex`. The WS backpressure bug is handled engine-side by the TCP relay in `engine/src/codex_ws_relay.rs` that fronts codex's app-server. Don't reintroduce the old `build-managed-codex.sh` / `managed-codex.patch` lane.
