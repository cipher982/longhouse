 ...
 * OikosChatPage Tests
 *
 * Tests for the Oikos chat page, specifically focusing on:
...
describe('OikosChatPage', () => {
  ...
  describe('Tool Call Hydration', () => {
    it('hydrates tool calls from API response into oikos tool store', async () => {
      ...
      renderOikosChatPage('/chat?thread=test-thread');
    ...
    it('hydrates multiple tool calls from a single message', async () => {
      ...
      renderOikosChatPage('/chat?thread=multi-tool-thread');
    ...
    it('handles messages without tool_calls gracefully', async () => {
      ...
      renderOikosChatPage('/chat?thread=no-tools-thread');
    ...
    it('clears tool store when loading a different thread', async () => {
      ...
      renderOikosChatPage('/chat?thread=new-thread');
    ...
    it('displays tool cards for hydrated tools via ActivityStream', async () => {
      ...
      renderOikosChatPage('/chat?thread=display-test');
  ...
  describe('Regression: Tool calls persist after page refresh', () => {
    it('shows tool calls when page is loaded with thread param (simulates refresh)', async () => {
       ...
       * 2. But OikosChatPage wasn't extracting them or setting runId
       * 3. oikosToolStore was empty (no SSE events to populate it)
       * 4. So ChatContainer found no tools to render
       *
       * The fix hydrates tool_calls from the API into the oikos tool store.
       */
      const now = new Date().toISOString();
      ...
      renderOikosChatPage('/chat?thread=refresh-test');
