/**
 * Connector Credentials Panel for Fiche Settings.
 *
...
export function ConnectorCredentialsPanel({ ficheId }: ConnectorCredentialsPanelProps) {
  const { data: connectors, isLoading, refetch } = useFicheConnectors(ficheId);
  const configureConnector = useConfigureConnector(ficheId);
  const deleteConnector = useDeleteConnector(ficheId);
  const testConnector = useTestConnector(ficheId);
  ...
  const handleDelete = async (connector: ConnectorStatus) => {
    const confirmed = await confirm({
      ...
      message: 'This fiche will no longer have access to this integration.',
      confirmLabel: 'Remove',
      cancelLabel: 'Keep',
      variant: 'danger',
    });
