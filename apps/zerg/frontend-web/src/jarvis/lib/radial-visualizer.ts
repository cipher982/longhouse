export type RadialVisualizerOptions = {
  ...
  outerInset?: number;
  /** Minimum radius for the visualizer ring */
  minRadius?: number;
};
...
export class RadialVisualizer {
  ...
  constructor(container: HTMLElement, options: RadialVisualizerOptions = {}) {
    ...
    this.minRadius = options.minRadius ?? 45;
  ...
  async start(): Promise<void> {
    ...
    if (!this.audioCtx) {
      this.audioCtx = new (window.AudioContext || (window as { webkitAudioContext?: typeof AudioContext }).webkitAudioContext)();
    }
