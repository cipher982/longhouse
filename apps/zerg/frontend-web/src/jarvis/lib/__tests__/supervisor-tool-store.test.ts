...
import { supervisorToolStore } from '../supervisor-tool-store';
import type { SupervisorToolCall } from '../supervisor-tool-store';

describe('SupervisorToolStore', () => {
  beforeEach(() => {
  ...
  describe('tool lifecycle tracking', () => {
    it('tracks tool started event', () => {
      ...
      eventBus.emit('supervisor:tool_started', toolData);
  ...
  describe('filtering and querying', () => {
    it('filters tools by runId', () => {
      ...
      const run1Tools = supervisorToolStore.getToolsForRun(1);
      const run2Tools = supervisorToolStore.getToolsForRun(2);
    ...
    it('returns tools sorted by start time', () => {
      ...
      const tools = supervisorToolStore.getToolsForRun(1);
  ...
  describe('state management', () => {
    ...
    it('loads tools from persisted data', () => {
      const persistedTools: SupervisorToolCall[] = [
        {
          toolCallId: 'call-1',
          toolName: 'tool_a',
          status: 'completed',
      ...
      supervisorToolStore.loadTools(persistedTools);
    ...
    it('activates on supervisor:started event', () => {
    ...
    it('deactivates on supervisor:complete event after delay', () => {
      eventBus.emit('supervisor:started', {
        runId: 1,
        task: 'Test task',
        timestamp: Date.now(),
      ...
      expect(supervisorToolStore.getState().isActive).toBe(true);
      ...
      expect(supervisorToolStore.getState().isActive).toBe(false);
    });

    it('deactivates immediately on supervisor:error', () => {
      eventBus.emit('supervisor:started', {
        runId: 1,
        task: 'Test task',
        timestamp: Date.now(),
    ...
    it('clears active state on supervisor:cleared', () => {
      eventBus.emit('supervisor:started', {
        runId: 1,
        task: 'Test task',
        timestamp: Date.now(),
      ...
      eventBus.emit('supervisor:cleared', {
        timestamp: Date.now(),
      });
  ...
  describe('live duration ticker', () => {
    it('starts ticker for running tools', () => {
      ...
      supervisorToolStore.subscribe(listener);
  ...
  describe('subscription management', () => {
    ...
    it('allows unsubscribing', () => {
      const listener = vi.fn();
      const unsubscribe = supervisorToolStore.subscribe(listener);
