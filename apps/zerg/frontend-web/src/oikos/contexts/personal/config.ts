/**
 * Personal context configuration for Oikos
 ...
 * prompt from /api/oikos/bootstrap is preferred as it includes user context.
 */
function generateInstructions(tools: ToolConfig[]): string {
  ...
  return `You are Oikos. You provide voice I/O (transcription + turn-taking cues).

## Architecture (v2.1 One-Brain)

- Oikos (server) is the ONLY brain and generates all assistant responses.
...
These capabilities may exist server-side (via Oikos tools) depending on configuration:
${capabilityList}

If the user asks for something that requires tools, respond that the request will be handled by Oikos.`;
}
...
// Actual tool execution happens server-side via Oikos.
...
const toolsConfig: ToolConfig[] = [
  ...
  },
  {
    ...
    enabled: true,
    // Executed by Oikos via WHOOP connector
  },
  {
    name: 'search_notes',
    description: 'Search personal notes and knowledge base',
    enabled: true,
    // Executed by Oikos via Runner (Obsidian vault)
  }
];
