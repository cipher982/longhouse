/**
 * Centralized model configuration for Oikos.
 *
 ...
 * - OIKOS_REALTIME_MODEL: Override the TIER_1 realtime model
 * - OIKOS_REALTIME_MODEL_MINI: Override the TIER_2 realtime model
 * - OIKOS_USE_MINI_MODEL: Set to "true" to use TIER_2 model
 * - OIKOS_VOICE: Override default voice
 */
...
// Realtime model tiers (Oikos voice interface)
...
export function getModelConfig(): ModelConfig {
  const realtimeModel = resolveModelName(getEnv('OIKOS_REALTIME_MODEL') || REALTIME_TIER_1);
  const realtimeModelMini = resolveModelName(getEnv('OIKOS_REALTIME_MODEL_MINI') || REALTIME_TIER_2);
  const useMiniModel = getEnv('OIKOS_USE_MINI_MODEL') === 'true';
  ...
  return {
    ...
    defaultVoice: getEnv('OIKOS_VOICE') || realtimeConfig.defaultVoice,
  };
}
