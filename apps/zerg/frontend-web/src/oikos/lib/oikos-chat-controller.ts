/**
 * OikosChatController - Manages text chat with Oikos via Zerg backend
 *
 * Responsibilities:
...
import {
  ...
  type OikosStartedPayload,
  type OikosThinkingPayload,
  type OikosTokenPayload,
  type OikosCompletePayload,
  type OikosDeferredPayload,
...
export interface OikosChatConfig {
  maxRetries?: number;
  retryDelay?: number;
}
...
export class OikosChatController {
  ...
  async initialize(): Promise<void> {
    logger.debug('[OikosChat] Initialized');
  }
  ...
  async loadHistory(limit: number = 50): Promise<OikosChatMessage[]> {
    try {
      ...
      logger.error('[OikosChat] Failed to load history:', _error);
      throw _error;
    }
  }

  /**
   * Send a text message to the Oikos and handle SSE response stream
   */
  async sendMessage(text: string, messageId: string, options?: { model?: string; reasoning_effort?: string; replay_scenario?: string }): Promise<void> {
    ...
    logger.debug(`[OikosChat] Sending message (messageId=${messageId}, model=${options?.model}, reasoning=${options?.reasoning_effort}): ${trimmedText}`);
    ...
    try {
      ...
      logger.debug('[OikosChat] Message sent and stream completed');
    } catch (_error) {
      if (_error instanceof Error && _error.name === 'AbortError') {
        logger.debug('[OikosChat] Message stream aborted');
        return;
      }

      logger.error('[OikosChat] Failed to send message:', _error);
      throw _error;
    } finally {
      this.clearWatchdog();
  ...
  private startWatchdog(messageId: string): void {
    ...
    this.watchdogTimer = window.setTimeout(async () => {
      logger.warn(`[OikosChat] Watchdog timeout for ${messageId} - marking as deferred`);
  ...
  private async streamChatResponse(
    ...
    const url = toAbsoluteUrl(`${CONFIG.OIKOS_API_BASE}/chat`);

    logger.debug('[OikosChat] Initiating SSE stream to:', url);
    ...
    logger.debug(`[OikosChat] Response status: ${response.status} ${response.statusText}`);
    logger.debug('[OikosChat] Response headers:', {
  ...
  private async processSSEStream(body: ReadableStream<Uint8Array>, signal: AbortSignal): Promise<void> {
    ...
    logger.debug('[OikosChat] Starting to read SSE stream...');
    ...
    const processBuffer = async (): Promise<string> => {
      ...
      logger.debug(`[OikosChat] Processing ${parts.length} complete messages, remaining: ${remaining.length} chars`);
      ...
      for (const message of parts) {
        ...
        if (eventType && eventType !== 'oikos_token') {
          logger.debug(`[OikosChat] Processing SSE event: ${eventType} (id=${eventId})`);
        }
        ...
        if (data) {
          try {
            ...
            if (eventType && (SSE_EVENT_TYPES as readonly string[]).includes(eventType)) {
              ...
              logger.warn(`[OikosChat] Unknown SSE event type: ${eventType}`);
            }
          } catch (_error) {
            logger.warn('[OikosChat] Failed to parse SSE data:', { data, error: _error });
          }
        }
      }
    ...
    try {
      while (true) {
        ...
        if (done) {
          logger.debug(`[OikosChat] Stream ended after ${chunkCount} reads`);
        ...
        buffer += chunk;
        logger.debug(`[OikosChat] Received chunk #${chunkCount} (${chunk.length} chars)`);
  ...
  private async handleSSEEvent(eventType: SSEEventType, data: unknown): Promise<void> {
    ...
    if (eventType === 'heartbeat') {
      this.petWatchdog();
      logger.debug('[OikosChat] Heartbeat');
      return;
    }
    ...
    switch (eventType) {
      case 'oikos_started': {
        const payload = wrapper.payload as OikosStartedPayload;
        this.petWatchdog();
        logger.debug('[OikosChat] Oikos started, message_id:', payload.message_id);
        if (payload.run_id) {
          this.currentRunId = payload.run_id;
        }

        // Check if this is a continuation run (commis result being processed)
        ...
        if (isContinuation) {
          ...
          logger.debug('[OikosChat] Continuation run detected, will create new message on first token');
        } else {
          // Normal run: Client already generated messageId, just confirm it matches
        ...
        // Emit oikos started event for progress UI (include trace_id for debugging)
        if (this.currentRunId) {
      ...
      case 'oikos_thinking': {
        const payload = wrapper.payload as OikosThinkingPayload;
        this.petWatchdog();
        logger.debug('[OikosChat] Oikos thinking:', payload.message);
      ...
      case 'oikos_token': {
        const payload = wrapper.payload as OikosTokenPayload;
      ...
      case 'oikos_complete': {
        const payload = wrapper.payload as OikosCompletePayload;
        this.petWatchdog();
        logger.debug('[OikosChat] Oikos complete');
      ...
      case 'oikos_deferred': {
        const payload = wrapper.payload as OikosDeferredPayload;
        // Timeout migration: run continues in background, we show a friendly message
        this.clearWatchdog();
        logger.debug('[OikosChat] Oikos deferred (timeout migration)');
      ...
      case 'oikos_waiting': {
        const payload = wrapper.payload as OikosWaitingPayload;
        this.petWatchdog();
        logger.debug('[OikosChat] Oikos waiting (interrupt):', payload.job_id);
      ...
      case 'oikos_resumed': {
        const payload = wrapper.payload as OikosResumedPayload;
        this.petWatchdog();
        logger.debug('[OikosChat] Oikos resumed (interrupt)');
      ...
      case 'oikos_tool_started': {
        ...
        this.petWatchdog();
        logger.debug('[OikosChat] Oikos tool started:', payload.tool_name);
      ...
      case 'oikos_tool_progress': {
        const payload = wrapper.payload as OikosToolProgressPayload;
        this.petWatchdog();
        logger.debug('[OikosChat] Oikos tool progress:', payload.message);
        eventBus.emit('oikos:tool_progress', {
          runId: payload.run_id ?? this.currentRunId ?? 0,
          toolCallId: payload.tool_call_id,
          message: payload.message,
          level: payload.level,
      ...
      case 'oikos_tool_completed': {
        const payload = wrapper.payload as OikosToolCompletedPayload;
        this.petWatchdog();
        logger.debug('[OikosChat] Oikos tool completed:', payload.tool_name);
      ...
      case 'oikos_tool_failed': {
        const payload = wrapper.payload as OikosToolFailedPayload;
        this.petWatchdog();
        logger.warn(`[OikosChat] Oikos tool failed: ${payload.tool_name} - ${payload.error}`);
        eventBus.emit('oikos:tool_failed', {
          runId: payload.run_id ?? this.currentRunId ?? 0,
          toolName: payload.tool_name,
          toolCallId: payload.tool_call_id,
          durationMs: payload.duration_ms,
      ...
      default:
        logger.debug('[OikosChat] Unknown SSE event type:', { eventType, data });
    }
  }
  ...
  cancel(): void {
    if (this.currentAbortController) {
      logger.debug('[OikosChat] Cancelling current stream');
      this.currentAbortController.abort();
      this.currentAbortController = null;
    }
  ...
  async clearHistory(): Promise<void> {
    try {
      ...
      logger.debug('[OikosChat] Server-side history cleared');
    } catch (_error) {
      logger.error('[OikosChat] Failed to clear history:', _error);
      throw _error;
    }
  }
  ...
  async attachToRun(runId: number): Promise<void> {
    logger.debug(`[OikosChat] Attaching to run ${runId} (lastEventId=${this.lastEventId})...`);
    ...
    try {
      ...
      if (this.lastEventId > 0) {
        ...
        logger.debug(`[OikosChat] Resuming from event ID: ${this.lastEventId}`);
      }
      ...
      void this.processSSEStream(body, signal)
        .then(() => {
          logger.debug(`[OikosChat] Attached to run ${runId} and stream completed`);
        })
        .catch((error) => {
          if (error instanceof Error && error.name === 'AbortError') {
            logger.debug(`[OikosChat] Attach to run ${runId} aborted`);
            return;
          }
          logger.error(`[OikosChat] SSE stream error while attached to run ${runId}:`, error);
        })
        .finally(() => {
      ...
      logger.error(`[OikosChat] Failed to attach to run ${runId}:`, _error);
      throw _error;
    }
  }
  ...
  dispose(): void {
    this.cancel();
    logger.debug('[OikosChat] Disposed');
  }
}
