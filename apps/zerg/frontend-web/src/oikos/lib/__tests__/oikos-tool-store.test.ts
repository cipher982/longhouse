...
import { oikosToolStore } from '../oikos-tool-store';
import type { OikosToolCall } from '../oikos-tool-store';

describe('OikosToolStore', () => {
  beforeEach(() => {
  ...
  describe('tool lifecycle tracking', () => {
    it('tracks tool started event', () => {
      ...
      eventBus.emit('oikos:tool_started', toolData);
  ...
  describe('filtering and querying', () => {
    it('filters tools by runId', () => {
      ...
      const run1Tools = oikosToolStore.getToolsForRun(1);
      const run2Tools = oikosToolStore.getToolsForRun(2);

      expect(run1Tools).toHaveLength(2);
      expect(run2Tools).toHaveLength(1);
      expect(run1Tools[0].toolName).toBe('tool_a');
      expect(run1Tools[1].toolName).toBe('tool_c');
      expect(run2Tools[0].toolName).toBe('tool_b');
    });
    ...
    it('returns tools sorted by start time', () => {
      ...
      const tools = oikosToolStore.getToolsForRun(1);
  ...
  describe('state management', () => {
    it('clears tools on clearTools()', () => {
      ...
      expect(oikosToolStore.getState().tools.size).toBe(1);
    ...
    it('loads tools from persisted data', () => {
      const persistedTools: OikosToolCall[] = [
        {
          toolCallId: 'call-1',
          toolName: 'tool_a',
          status: 'completed',
      ...
      oikosToolStore.loadTools(persistedTools);
    ...
    it('activates on oikos:started event', () => {
    ...
    it('deactivates on oikos:complete event after delay', () => {
      eventBus.emit('oikos:started', {
        runId: 1,
        task: 'Test task',
        timestamp: Date.now(),
      ...
      expect(oikosToolStore.getState().isActive).toBe(true);
      ...
      expect(oikosToolStore.getState().isActive).toBe(false);
    });

    it('deactivates immediately on oikos:error', () => {
      eventBus.emit('oikos:started', {
        runId: 1,
        task: 'Test task',
        timestamp: Date.now(),
    ...
    it('clears active state on oikos:cleared', () => {
      eventBus.emit('oikos:started', {
        runId: 1,
        task: 'Test task',
        timestamp: Date.now(),
  ...
  describe('subscription management', () => {
    ...
    it('allows unsubscribing', () => {
      const listener = vi.fn();
      const unsubscribe = oikosToolStore.subscribe(listener);
