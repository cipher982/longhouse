...
describe('TimelineLogger', () => {
  ...
  it('should capture oikos lifecycle events', () => {
    const messageId = 'test-123'
    logger.setMessageId(messageId)
    ...
    // Oikos started
    eventBus.emit('oikos:started', { runId: 1, task: 'test task', timestamp: now + 100 })

    // Check events before complete (which triggers output and reset)
    let events = (logger as any).events as Array<any>
    expect(events.length).toBe(2)
    expect(events[0].phase).toBe('send')
    expect(events[1].phase).toBe('oikos_started')

    // Oikos complete (triggers output and reset)
  ...
  it('should output timeline on oikos:complete', () => {
    const messageId = 'test-123'
    logger.setMessageId(messageId)
    ...
    // Emit events
    eventBus.emit('text_channel:sent', { text: 'test', timestamp: now })
    eventBus.emit('oikos:started', { runId: 1, task: 'test', timestamp: now + 50 })
    eventBus.emit('oikos:complete', {
      runId: 1,
      result: 'Done',
      status: 'success',
  ...
  it('should calculate T+offset from first event', () => {
    ...
    eventBus.emit('text_channel:sent', { text: 'test', timestamp: now })
    eventBus.emit('oikos:started', { runId: 1, task: 'test', timestamp: now + 100 })
    eventBus.emit('oikos:complete', {
      runId: 1,
      result: 'Done',
      status: 'success',
