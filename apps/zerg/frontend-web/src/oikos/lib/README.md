...
| `state-manager.ts` | Chat/session state + assistant status updates | `useOikosApp`, `useTextChannel` |
| `oikos-chat-controller.ts` | SSE streaming for `POST /api/oikos/chat` | `useTextChannel` |
...
| `oikos-tool-store.ts` | Oikos tool card state | `ActivityStream` |
| `timeline-logger.ts` | Timeline logging (`?log=timeline`) | Performance/debug |
| `session-handler.ts` | OpenAI Realtime session management (voice I/O) | `useOikosApp` |
