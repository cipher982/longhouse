...
class CommisProgressStore {
  ...
  /**
   * Schedule a delayed clear (cancellable if new oikos starts)
   */
  ...
  setReconnecting(runId: number): void {
    logger.debug(`[CommisProgress] Reconnecting to run ${runId}...`);
    this.setState({
      isActive: true,
      currentRunId: runId,
      reconnecting: true,
  ...
  private subscribeToEvents(): void {
    ...
    eventBus.on('oikos:deferred', () => {
      this.handleDeferred();
    });

    eventBus.on('oikos:error', (data) => {
  ...
  private handleStarted(runId: number, task: string): void {
    ...
    logger.debug(`[CommisProgress] Tracking run ${runId}: ${task}`);
  }
  ...
  /**
   * Maybe schedule clear if oikos is done
   */
  private maybeScheduleClear(): void {
    if (!this.state.oikosDone) return;
  ...
  /**
   * Handle oikos complete
   */
  private handleComplete(runId: number, result: string, status: string): void {
    logger.debug(`[CommisProgress] Complete: ${runId} (${status})`);
    this.setState({ oikosDone: true, reconnecting: false });
    this.maybeScheduleClear();
  }

  /**
   * Handle oikos deferred
   */
