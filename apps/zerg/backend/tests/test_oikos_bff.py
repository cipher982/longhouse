"""Tests for Oikos BFF (Backend-for-Frontend) endpoints.

Tests the proxy endpoints and bootstrap endpoint that make zerg-backend
the single API surface for Oikos.
"""
...
class TestBootstrapEndpoint:
    """Tests for GET /api/oikos/bootstrap."""
    ...
    def test_bootstrap_returns_prompt_and_tools(self, client, test_user, db_session):
        ...
        # Check tools list (v2.1: route_to_oikos removed)
    ...
    def test_bootstrap_filters_disabled_tools(self, client, test_user, db_session):
        ...
        # v2.1: route_to_oikos has been removed, only 3 tools remain
        test_user.context = {
            "display_name": "David",
            "tools": {
                "location": True,
        ...
        # Check only enabled tools are returned (v2.1: route_to_oikos removed)
        tool_names = [t["name"] for t in data["enabled_tools"]]
        assert "get_current_location" in tool_names
    ...
    def test_bootstrap_defaults_to_all_tools_enabled(self, client, test_user, db_session):
        ...
        # Should have all 3 tools (v2.1: route_to_oikos removed)
        tool_names = [t["name"] for t in data["enabled_tools"]]
        assert "get_current_location" in tool_names
...
class TestSessionEndpoint:
    """Tests for /api/oikos/session endpoint (direct OpenAI integration)."""
    ...
    def test_session_returns_token(self, client):
        ...
        with patch("zerg.services.openai_realtime.httpx.AsyncClient") as mock_client_class:
            ...
            response = client.get("/api/oikos/session")
...
# NOTE: /api/oikos/tool endpoint was removed - MCP tools were disabled in production
# and tool execution now goes through the oikos/commis system.
...
class TestBootstrapPromptIntegration:
    ...
    def test_bootstrap_tools_match_prompt_claims(self, client, test_user, db_session):
        ...
        # Should have the expected tools (v2.1: route_to_oikos removed)
        assert "get_current_location" in tool_names
        assert "get_whoop_data" in tool_names
        assert "search_notes" in tool_names
...
class TestChatEndpoint:
    """Tests for POST /api/oikos/chat endpoint."""
    ...
    def test_chat_requires_auth(self, client):
        ...
        auth.AUTH_DISABLED = False
        try:
            response = client.post("/api/oikos/chat", json={"message": "Hello"})
            assert response.status_code == 401
        finally:
            auth.AUTH_DISABLED = prev
    ...
    def test_chat_validates_message_required(self, client):
        ...
        response = client.post("/api/oikos/chat", json={})
        assert response.status_code == 422  # Validation error


class TestHistoryEndpoint:
    """Tests for GET /api/oikos/history endpoint."""
    ...
    def test_history_requires_auth(self, client):
        ...
        auth.AUTH_DISABLED = False
        try:
            response = client.get("/api/oikos/history")
            assert response.status_code == 401
        finally:
            auth.AUTH_DISABLED = prev
    ...
    def test_history_returns_messages(self, client, test_user, db_session):
        ...
        # Create oikos thread with messages
        oikos_service = OikosService(db_session)
        fiche = oikos_service.get_or_create_oikos_fiche(test_user.id)
        thread = oikos_service.get_or_create_oikos_thread(test_user.id, fiche)
    ...
    def test_history_pagination(self, client, test_user, db_session):
        ...
        # Create oikos thread with multiple messages
        oikos_service = OikosService(db_session)
        fiche = oikos_service.get_or_create_oikos_fiche(test_user.id)
        thread = oikos_service.get_or_create_oikos_thread(test_user.id, fiche)
        ...
        response = client.get("/api/oikos/history?limit=3&offset=0")
        data = response.json()
        ...
        # Get second page
        response = client.get("/api/oikos/history?limit=3&offset=3")
        data = response.json()
    ...
    def test_history_filters_system_messages(self, client, test_user, db_session):
        ...
        # Create oikos thread (has system message by default)
        oikos_service = OikosService(db_session)
        fiche = oikos_service.get_or_create_oikos_fiche(test_user.id)
        thread = oikos_service.get_or_create_oikos_thread(test_user.id, fiche)
