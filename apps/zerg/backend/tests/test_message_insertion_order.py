...
def test_interleaved_order_after_delay(client: TestClient, db_session) -> None:  # noqa: D401
    ...
    fiche_id = _create_agent(client)
    thread_id = _create_thread(client, fiche_id)

    # First exchange
    _send_user_message(client, thread_id, "Hello")
    # Instead of running the fiche, directly insert an assistant message
...
def _create_thread(client: TestClient, fiche_id: int) -> int:
    resp = client.post(
        "/api/threads",
        json={"fiche_id": fiche_id, "title": "demo", "active": True},
    )
    resp.raise_for_status()
    return resp.json()["id"]
