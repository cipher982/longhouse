"""Tests for Oikos run status and stream endpoints (Phase 4 of durable-runs-v2.2)."""
...
class TestGetRunStatus:
    """Tests for GET /api/oikos/runs/{run_id} endpoint."""

    @pytest.fixture
    def run_components(self, db_session, test_user):
        """Create oikos fiche, thread, run, and messages for testing."""
    ...
    def test_get_running_run_status(self, client, run_components):
        ...
        response = client.get(f"/api/oikos/runs/{run.id}")
    ...
    def test_get_nonexistent_run(self, client):
        ...
        response = client.get("/api/oikos/runs/99999")
    ...
    def test_get_run_multi_tenant_isolation(self, client, db_session, run_components, other_user):
        ...
        other_fiche = OikosService(db_session).get_or_create_oikos_fiche(other_user.id)
        other_thread = OikosService(db_session).get_or_create_oikos_thread(other_user.id, other_fiche)
        ...
        response = client.get(f"/api/oikos/runs/{other_run.id}")
...
class TestListOikosRuns:
    def test_list_runs_avoids_fiche_n_plus_one(self, client, db_session, test_user, monkeypatch):
        service = OikosService(db_session)
        fiche = service.get_or_create_oikos_fiche(test_user.id)
        thread = service.get_or_create_oikos_thread(test_user.id, fiche)
        ...
        real_selectinload = oikos_runs.selectinload

        def _track_selectinload(*args, **kwargs):
            selectinload_called["value"] = True
            return real_selectinload(*args, **kwargs)

        monkeypatch.setattr(oikos_runs, "selectinload", _track_selectinload)

        response = client.get("/api/oikos/runs")
        assert response.status_code == status.HTTP_200_OK
...
class TestAttachToRunStream:
    """Tests for GET /api/oikos/runs/{run_id}/stream endpoint."""

    @pytest.fixture
    def run_components(self, db_session, test_user):
        """Create oikos fiche, thread, and run for stream testing."""
        service = OikosService(db_session)
        fiche = service.get_or_create_oikos_fiche(test_user.id)
        thread = service.get_or_create_oikos_thread(test_user.id, fiche)
    ...
    async def test_attach_to_completed_run(self, async_client, db_session, run_components):
        ...
        response = await async_client.get(f"/api/oikos/runs/{run.id}")
    ...
    async def test_attach_to_nonexistent_run(self, async_client):
        ...
        # Use the JSON endpoint, not the SSE stream endpoint
        response = await async_client.get("/api/oikos/runs/99999")
    ...
    async def test_attach_stream_multi_tenant_isolation(self, async_client, db_session, run_components, other_user):
        ...
        response = await async_client.get(f"/api/oikos/runs/{other_run.id}")
...
class TestGetRunTimeline:
    """Tests for GET /api/oikos/runs/{run_id}/timeline endpoint (Phase 3: chat-observability-eval)."""
    ...
    def run_with_events(self, db_session, test_user):
        ...
        # Create timeline events (simulating a full oikos + commis flow)
        ...
        events = [
            RunEvent(
                ...
                payload={"message": "Starting oikos"},
        ...
        return {"fiche": fiche, "thread": thread, "run": run, "events": events}
    ...
    def test_get_timeline_success(self, client, run_with_events):
        ...
        response = client.get(f"/api/oikos/runs/{run.id}/timeline")
        ...
        assert events[0]["phase"] == "oikos_started"
        assert events[0]["offset_ms"] == 0
        assert events[0]["timestamp"] is not None

        # Last event should have highest offset
        assert events[-1]["phase"] == "oikos_complete"
    ...
    def test_get_timeline_summary_calculations(self, client, run_with_events):
        ...
        # Oikos thinking time: oikos_started to commis_spawned
        assert summary["oikos_thinking_ms"] == 500
    ...
    def test_get_timeline_empty_events(self, client, db_session, test_user):
        ...
        assert data["summary"]["oikos_thinking_ms"] is None
    ...
    def test_get_timeline_nonexistent_run(self, client):
        ...
        response = client.get("/api/oikos/runs/99999/timeline")
    ...
    def test_get_timeline_multi_tenant_isolation(self, client, db_session, run_with_events, other_user):
        ...
        response = client.get(f"/api/oikos/runs/{other_run.id}/timeline")
    ...
    def test_get_timeline_partial_flow(self, client, db_session, test_user):
        ...
        # Only oikos events (no commis spawned)
        base_time = datetime.now(timezone.utc)
        events = [
            RunEvent(
                run_id=run.id,
        ...
        assert summary["oikos_thinking_ms"] is None  # No commis_spawned event
