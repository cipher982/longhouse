...
class TestExtractSummaryFromOutput:
    ...
    def test_failed_with_error_message(self):
        ...
        result = _extract_summary_from_output(
            output=None, status="failed", error="Connection refused: could not connect to database"
        )
    ...
    def test_failed_with_long_error_truncated(self):
        ...
        error = "E" * 200
        result = _extract_summary_from_output(output=None, status="failed", error=error)
        assert result.startswith("[FAILED] ")
        assert len(result) <= 153
        assert result.endswith("...")
    ...
    def test_failed_no_error_uses_output(self):
        ...
        result = _extract_summary_from_output(output="Some partial output before failure", status="failed", error=None)
        assert result.startswith("[FAILED] ")
        assert "partial output" in result

    def test_failed_no_error_no_output(self):
        """Failed status with no error and no output should use fallback."""
        result = _extract_summary_from_output(output=None, status="failed", error=None)
    ...
    def test_failed_empty_error_uses_output(self):
        ...
        result = _extract_summary_from_output(output="Partial output", status="failed", error="")
        assert result.startswith("[FAILED] ")
        assert "Partial output" in result
    ...
    def test_timeout_with_error_message(self):
        ...
        result = _extract_summary_from_output(output=None, status="timeout", error="Execution timed out after 300s")
    ...
    def test_timeout_no_error_uses_output(self):
        ...
        result = _extract_summary_from_output(output="Processing took too long", status="timeout", error=None)
        assert result.startswith("[TIMEOUT] ")
        assert "Processing" in result

    def test_timeout_no_error_no_output(self):
        """Timeout with no error and no output should use fallback."""
        result = _extract_summary_from_output(output=None, status="timeout", error=None)
    ...
    def test_cancelled_with_error_message(self):
        ...
        result = _extract_summary_from_output(output=None, status="cancelled", error="Cancelled by user")
    ...
    def test_cancelled_no_error_uses_output(self):
        ...
        result = _extract_summary_from_output(output="Partial work done", status="cancelled", error=None)
        assert result.startswith("[CANCELLED] ")
    ...
    def test_cancelled_no_error_no_output(self):
        ...
        result = _extract_summary_from_output(output=None, status="cancelled", error=None)
        ...
        for status in ["success", "failed", "timeout", "cancelled"]:
            for output in [None, "x" * 200]:
                for error in [None, "e" * 200]:
                    result = _extract_summary_from_output(output=output, status=status, error=error)
    ...
    def test_error_cleans_newlines(self):
        ...
        result = _extract_summary_from_output(output=None, status="failed", error="Line 1\nLine 2\nLine 3")
        assert "\n" not in result
    ...
    def test_priority_error_over_output(self):
        ...
        result = _extract_summary_from_output(output="This is the output", status="failed", error="This is the error")
