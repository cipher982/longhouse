...
@pytest.mark.timeout(30)
class TestSupervisorReActEngineInterrupt:
    """Test supervisor ReAct engine interrupt handling."""

    @pytest.mark.asyncio
    async def test_spawn_worker_triggers_interrupt(self, db_session, test_user, sample_agent):
        """Test that spawn_worker tool results in interrupted SupervisorResult."""
        from langchain_core.messages import HumanMessage
        ...
        # Create a minimal tool list with mocked spawn_worker behavior
        class MockSpawnWorkerTool(BaseTool):
        ...
        messages = [
            ...
            HumanMessage(content="Run a background task"),
        ]
        ...
        with (
            patch(
                "zerg.services.supervisor_react_engine._call_llm",
            ...
            result = await run_supervisor_loop(
                messages=messages,
                agent_row=sample_agent,
                tools=[MockSpawnWorkerTool()],
                run_id=1,
                owner_id=test_user.id,
            )
