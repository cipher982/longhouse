...
async def test_oikos_commis_interrupt_resume_flow(
    db_session,
    test_user,
    ...
    # Create a run record the same way /api/oikos/chat does (run_id known before streaming).
    run = Run(
        fiche_id=fiche.id,
        thread_id=thread.id,
    ...
    # Start consuming SSE stream BEFORE running the oikos to avoid missing early events.
    events: list[dict] = []

    async def consume_stream() -> None:
        async for evt in stream_run_events(run.id, test_user.id):
            events.append(evt)
            if evt.get("event") == "oikos_complete":
                break
    ...
    # Test Phase 1: Oikos run should become WAITING when interrupted
    ...
    async def mock_resume(db, run_id, commis_result):
        run_to_update = db.query(Run).filter(Run.id == run_id).first()
        run_to_update.status = RunStatus.SUCCESS
        db.commit()

        # Emit oikos_complete event (same as real resume does)
        await emit_run_event(
            db=db,
            run_id=run_id,
    ...
    # Wait for the stream to receive oikos_complete
    try:
...
async def test_spawn_commis_fallback_when_outside_runnable_context(
    ...
    """Test that spawn_commis queues a job when called outside oikos context.

    This tests the graceful degradation when spawn_commis is called directly
    (e.g., from tests or CLI) rather than from within the oikos loop.
    """
    from zerg.tools.builtin.oikos_tools import spawn_commis_async

    # Set up oikos context for the tool
    service = OikosService(db_session)
    fiche = service.get_or_create_oikos_fiche(test_user.id)
    thread = service.get_or_create_oikos_thread(test_user.id, fiche)
    ...
    # Set oikos context (normally done by oikos_service)
...
async def test_resume_completes_interrupted_run(
    ...
    """Test that resume_oikos_with_commis_result completes an interrupted run.
...
async def test_different_tasks_create_separate_commis(
    ...
    # Set oikos context for spawn_commis tool
    sup_token = set_oikos_context(
        run_id=run.id,
        owner_id=test_user.id,
    ...
    try:
        ...
        commis = db_session.query(CommisJob).filter(CommisJob.oikos_run_id == run.id).all()
        ...
        commis_after = db_session.query(CommisJob).filter(CommisJob.oikos_run_id == run.id).all()
