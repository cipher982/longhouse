"""LangGraph-free ReAct engine for concierge fiches.

This module provides a pure async ReAct loop for concierge execution without
...
- No interrupt() - spawn_commis raises CourseInterrupted directly
...
class ConciergeResult:
    """Result from concierge ReAct loop execution."""
    ...
    *,
    ...
    if is_test_model(model):
        ...
        if model == "gpt-scripted":
            # ScriptedChatLLM not typically used for concierge
    ...
    try:
        while not heartbeat_cancelled.is_set():
            ...
            if course_id is not None:
                ...
                logger.debug(f"Emitted heartbeat for concierge run {course_id} during {phase}")
...
async def _call_llm(
    ...
    *,
    ...
    heartbeat_cancelled = asyncio.Event()
    heartbeat_task = asyncio.create_task(_emit_heartbeats(heartbeat_cancelled, course_id, owner_id, phase))
...
async def _execute_tool(
    ...
    *,
    ...
    For spawn_commis, raises CourseInterrupted instead of returning ToolMessage.
    ...
    Raises:
        CourseInterrupted: If spawn_commis is called and job is queued (not already complete).
    """
    import json
    ...
    if not tool_to_call:
        ...
        try:
            ...
            if tool_name == "spawn_commis":
                ...
                if isinstance(job_result, dict):
                    ...
                    if job_result.get("status") == "queued" and job_id is not None:
                        ...
                        # Raise interrupt to pause concierge
            ...
            # Special handling for spawn_workspace_commis - needs interrupt handling
                ...
                from zerg.tools.builtin.concierge_tools import spawn_workspace_commis_async
    ...
    settings = get_settings()
    max_chars = max(0, int(settings.concierge_tool_output_max_chars or 0))
    preview_chars = max(0, int(settings.concierge_tool_output_preview_chars or 0))
...
async def _execute_tools_parallel(
    ...
    *,
    ...
    - Caller (concierge_service) creates CommisBarrier + flips to 'queued'
    ...
    Note:
        Does NOT raise CourseInterrupted - caller handles interruption.
    """
    ...
    if other_calls:
        ...
        # CRITICAL: Check for CourseInterrupted first - it must propagate, not become a ToolMessage
        from zerg.managers.fiche_runner import CourseInterrupted

        for tc, result in zip(other_calls, results):
            if isinstance(result, CourseInterrupted):
                # Re-raise interrupt (e.g., from wait_for_commis)
                # This ensures the concierge enters WAITING state
                logger.info(f"[INTERRUPT] Re-raising CourseInterrupted from {tc.get('name')}")
                raise result
    ...
    if spawn_calls:
        ...
        for tc in spawn_calls:
            ...
            try:
                ...
                existing_job = None
                if tool_call_id and concierge_course_id:
                    existing_job = (
                        db.query(CommisJob)
                        .filter(
        ...
        # Commis run in background, concierge sees results via inbox context on next turn
        if created_jobs:
...
async def run_concierge_loop(
    ...
    *,
    ...
    """Run the concierge ReAct loop until completion or interrupt.

    This is the main entry point for LangGraph-free concierge execution.
    ...
    Returns:
        ConciergeResult with messages, usage, and interrupt status.
    ...
    try:
        ...
        while isinstance(llm_response, AIMessage) and llm_response.tool_calls:
            ...
            if iteration > MAX_REACT_ITERATIONS:
                ...
                error_msg = AIMessage(
                    content=(
                        f"Error: Concierge exceeded maximum of {MAX_REACT_ITERATIONS} "
