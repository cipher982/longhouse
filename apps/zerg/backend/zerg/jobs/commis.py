"""Queue commis that executes jobs from durable queue.

This module provides a background commis loop that:
...
async def run_queue_commis(poll_seconds: float | None = None) -> None:
    """Main commis loop: claim and execute jobs."""
    owner = default_owner()
    poll_seconds = poll_seconds or DEFAULT_POLL_SECONDS

    logger.info(f"Job queue commis started as {owner.name}")
    ...
    while True:
        try:
            ...
            logger.error(f"Commis loop error: {e}", exc_info=True)
            await asyncio.sleep(poll_seconds)
