"""OikosEmitter - emits oikos_tool_* events with identity baked in.

This class replaces the contextvar-based event emission pattern. The emitter's
identity (oikos) is fixed at construction time, so it always emits the
...
@dataclass
class OikosEmitter:
    """Emitter for oikos tool events - identity baked at construction.

    Always emits oikos_tool_* events, regardless of contextvar state.
    This eliminates the contextvar leakage bug where commis events could
    contaminate oikos event emission.
    ...
    ----------
    run_id
        Oikos Run ID for event correlation
    owner_id
        User ID that owns this oikos fiche
    message_id
    ...
    def is_commis(self) -> bool:
        """Always False - this is a oikos emitter."""
        return False

    @property
    def is_oikos(self) -> bool:
        """Always True - this is a oikos emitter."""
        return True
    ...
    async def emit_tool_started(
        ...
        """Emit oikos_tool_started event.

        Always emits oikos_tool_started - identity is fixed at construction.
        """
        ...
        try:
            ...
            logger.warning("Failed to emit oikos_tool_started event", exc_info=True)
    ...
    async def emit_tool_completed(
        ...
        """Emit oikos_tool_completed event.

        Always emits oikos_tool_completed - identity is fixed at construction.
        """
        from zerg.services.event_store import append_run_event
        ...
        try:
            ...
            await append_run_event(
                run_id=self.run_id,
                event_type="oikos_tool_completed",
                payload={
                    "owner_id": self.owner_id,
                    "tool_name": tool_name,
                    "tool_call_id": tool_call_id,
            ...
            logger.warning("Failed to emit oikos_tool_completed event", exc_info=True)
    ...
    async def emit_tool_failed(
        ...
        """Emit oikos_tool_failed event.

        Always emits oikos_tool_failed - identity is fixed at construction.
        """
        from zerg.services.event_store import append_run_event
        ...
        try:
            ...
            logger.warning("Failed to emit oikos_tool_failed event", exc_info=True)
    ...
    async def emit_heartbeat(
        ...
        """Emit oikos_heartbeat event during long-running LLM calls.

        Always emits oikos_heartbeat - identity is fixed at construction.
        """
        ...
        try:
            ...
            logger.debug(f"Emitted heartbeat for oikos run {self.run_id} during {phase}")
        except Exception:
