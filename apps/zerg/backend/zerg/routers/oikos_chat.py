...
async def _chat_stream_generator(
    ...
    Subscribes to oikos events and streams assistant responses.
    The background task is started from within this generator to avoid race conditions.

    IMPORTANT: We must not start the oikos before the SSE stream has subscribed
    to events, otherwise early events (e.g. oikos_started) can be missed.
    """
    ...
    # Stream events, and only start the oikos AFTER the stream generator has
    ...
    async for event in stream_run_events(run_id, owner_id):
        ...
        if not started_background:
            ...
            logger.info(f"Chat SSE: starting background oikos for run {run_id}", extra={"tag": "OIKOS"})
            task_handle = asyncio.create_task(run_oikos_background())
            await _register_oikos_task(run_id, task_handle)
...
async def oikos_chat(
    ...
    """Text chat endpoint - streams responses from Oikos.
    ...
    logger.info(
        f"Oikos chat: created run {run_id} for user {current_user.id}, "
    ...
    if request.replay_scenario and is_replay_enabled():
        logger.info(
            f"Oikos chat: using REPLAY MODE for run {run_id}, scenario={request.replay_scenario}",
            extra={"tag": "OIKOS"},
        )
