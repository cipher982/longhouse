"""Oikos Integration API Router.

This module serves as the main orchestrator for Oikos endpoints, importing
...
- oikos_run_dispatch: Oikos run dispatch + cancel
...
async def _oikos_event_generator(_current_user):
    ...
    try:
        ...
        yield {
            "event": "connected",
            "data": json.dumps({"message": "Oikos SSE stream connected"}),
        }
...
async def oikos_events(
    ...
    """
    return EventSourceResponse(_oikos_event_generator(current_user))
...
def _fetch_commis_activity(db: Session, fiche_id: int, tool_call_ids: list[str]) -> dict[str, dict]:
    ...
    run_ids = [row.id for row in db.query(Run.id).filter(Run.fiche_id == fiche_id).all()]
    if not run_ids:
        return {}
    ...
    # Use oikos_tool_started -> commis_spawned correlation only when tool_call_id is missing.
...
class OikosBootstrapResponse(BaseModel):
    ...
    prompt: str = Field(..., description="Complete Oikos system prompt")
    ...
    available_models: List[OikosModelInfo] = Field(..., description="Models available for selection")
    preferences: OikosPreferences = Field(..., description="User's saved preferences")
...
def oikos_bootstrap(
    ...
    # These are now Oikos-owned tools, NOT Realtime tools.
    # Realtime has zero tools (I/O only: transcription + VAD).
    # All user input goes to Oikos via POST /api/oikos/chat.
    # Oikos can call these tools directly using connector credentials.
    AVAILABLE_TOOLS = {
    ...
    preferences = OikosPreferences(chat_model=requested_model, reasoning_effort=requested_effort)
...
def oikos_update_preferences(
    ...
    """Update user's Oikos preferences.
