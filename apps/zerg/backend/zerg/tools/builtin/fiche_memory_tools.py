"""Fiche memory tools for persistent key-value storage.

These tools allow fiches to store and retrieve persistent data across
...
class MemorySetInput(BaseModel):
    """Input schema for fiche_memory_set."""
    ...
    try:
        ...
        if not user_id:
            return tool_error(
                ...
                user_message="No user context available. This tool can only be used within a fiche execution.",
            )
        ...
        with db_session() as db:
            ...
            entry = db.query(FicheMemoryKV).filter(FicheMemoryKV.user_id == user_id, FicheMemoryKV.key == key.strip()).first()
...
class MemoryGetInput(BaseModel):
    """Input schema for fiche_memory_get."""
...
def fiche_memory_get(
    ...
    Example:
        >>> fiche_memory_get(key="user_preferences")
        {"ok": True, "data": {"key": "user_preferences", "value": {...}}}

        >>> fiche_memory_get(tags=["settings", "ui"], limit=10)
    ...
    try:
        ...
        with db_session() as db:
            ...
            query = db.query(FicheMemoryKV).filter(FicheMemoryKV.user_id == user_id)
...
class MemoryDeleteInput(BaseModel):
    """Input schema for fiche_memory_delete."""
...
def fiche_memory_delete(
    ...
    Example:
        >>> fiche_memory_delete(key="temp_data")
        {"ok": True, "data": {"deleted_count": 1}}

        >>> fiche_memory_delete(tags=["temporary", "cache"])
    ...
    try:
        ...
        with db_session() as db:
            ...
            if key:
                result = db.query(FicheMemoryKV).filter(FicheMemoryKV.user_id == user_id, FicheMemoryKV.key == key.strip()).delete()
            ...
            if tags:
                ...
                all_entries = db.query(FicheMemoryKV).filter(FicheMemoryKV.user_id == user_id).all()
                deleted_count = 0
                for entry in all_entries:
                    entry_tags = entry.tags or []
                    if any(tag in entry_tags for tag in tags):
...
class MemoryExportInput(BaseModel):
    """Input schema for fiche_memory_export."""

    pass


def fiche_memory_export() -> Dict[str, Any]:
...
TOOLS = [
    StructuredTool.from_function(
        func=fiche_memory_set,
        name="fiche_memory_set",
        description=(
    ...
    StructuredTool.from_function(
        func=fiche_memory_get,
        name="fiche_memory_get",
        description=(
            "Retrieve memory entries by key or tags. "
    ...
    StructuredTool.from_function(
        func=fiche_memory_delete,
        name="fiche_memory_delete",
        description=(
            "Delete memory entries by key or tags. "
    ...
    StructuredTool.from_function(
        func=fiche_memory_export,
        name="fiche_memory_export",
        description=(
            "Export all memory entries for the user. "
