"""Tests for _is_sqlite_url helper function.
...
class TestIsSqliteUrl:
    """Test _is_sqlite_url() properly detects SQLite URLs."""

    def test_unquoted_sqlite_url(self):
        """Unquoted sqlite URL should be detected."""
        assert _is_sqlite_url("sqlite:///path/to/db.sqlite") is True
    ...
    def test_postgres_url_not_sqlite(self):
        ...
        assert _is_sqlite_url("postgresql://user:pass@host:5432/db") is False
        assert _is_sqlite_url("postgresql+psycopg://user:pass@host/db") is False
        assert _is_sqlite_url("postgres://user:pass@host:5432/db") is False
    ...
    def test_empty_url(self):
        ...
        assert _is_sqlite_url("") is False
        assert _is_sqlite_url(None) is False
        assert _is_sqlite_url("   ") is False
        assert _is_sqlite_url('""') is False
        assert _is_sqlite_url("''") is False
    ...
    def test_sqlite_with_parameters(self):
        ...
        assert _is_sqlite_url("sqlite:///test.db?mode=ro") is True
        assert _is_sqlite_url("sqlite:///test.db?mode=rwc&cache=shared") is True
