"""
Migration script to promote fiche-level credentials to account-level.

Usage:
    python scripts/migrate_fiche_credentials.py [--dry-run]

Logic:
1. Iterate through all users.
2. For each user, group their fiche credentials by connector type.
3. For each connector type:
    ...
    - Collect all fiche credentials for this type.
    - If no fiche credentials, skip.
    - Determine the "best" credential to promote:
        - The one used by the most fiches?
        - Or the most recently updated? (PRD suggests most recently updated)
    - Create AccountConnectorCredential with this value.
    - For ALL fiches that had this EXACT SAME encrypted value:
        - Delete their fiche-level credential (they will now inherit).
    - For fiches with DIFFERENT values:
        - Keep their fiche-level credential (as an override).
...
def migrate_credentials(db: Session, dry_run: bool = False):
    ...
    for user in users:
        ...
        # Get all credentials for this user's fiches
        # Join Fiche to get owner_id
        fiche_creds = (
            db.query(ConnectorCredential, Fiche)
            .join(Fiche, ConnectorCredential.fiche_id == Fiche.id)
            .filter(Fiche.owner_id == user.id)
            .all()
        )
        ...
        by_type: Dict[str, List[tuple[ConnectorCredential, Fiche]]] = defaultdict(list)
        for cred, fiche in fiche_creds:
            by_type[cred.connector_type].append((cred, fiche))
            ...
            for cred, fiche in items:
                if cred.encrypted_value == best_cred.encrypted_value:
                    ...
                    if not dry_run:
                        db.delete(cred)
                    total_deleted += 1
                    logger.info(f"    -> Deleting redundant cred for Fiche {fiche.id}")
                else:
                    # Different value - keep as override
                    total_kept_override += 1
                    logger.info(f"    -> Keeping distinct override for Fiche {fiche.id}")
    ...
    logger.info(f"  Deleted (redundant fiche creds): {total_deleted}")
    logger.info(f"  Kept (overrides): {total_kept_override}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Migrate fiche credentials to account level")
