...
Traccar GPS tracking server is running on the clifford VPS to enable Jarvis personal tools to access location data. This replaces the previous unreliable local setup at REDACTED_LOCAL_IP:5055.

## Deployment Details

### Server Location

- **Host**: clifford (REDACTED_IP)
- **Container**: `traccar-server`
- **Web UI**: http://REDACTED_IP:5055
- **API Base**: http://REDACTED_IP:5055/api
...
- **Server URL**: http://REDACTED_IP:5055
...
```bash
# From laptop
curl -u admin:YOUR_PASSWORD http://REDACTED_IP:5055/api/positions
```
...
auth = ("admin", "password")
response = httpx.get("http://REDACTED_IP:5055/api/positions", auth=auth)
```
