...
1.  **Dual Streaming Paths:** Redundant mechanisms for SSE (Chat/Oikos) vs. WebSockets (Dashboard/Runners).
...
*   **Write Path:** All system events (Commis, Oikos, Tool) MUST be persisted to DB via `EventStore.emit_run_event`.
...
2.  **No Context Leakage:** Do not use `contextvars` to transport request-scoped identifiers (`run_id`, `commis_id`) across async boundaries.
3.  **Persistence First:** If it's not in `RunEvent` table, it didn't happen.
...
*   **Action:** Audit `CommisRunner` and `OikosService`. Ensure *every* event emission goes through `EventStore.emit_run_event`.
*   **Action:** Remove direct `event_bus.publish` calls for run events.

### Step 3: Explicit Context (Stability)
*   **Action:** Update `CommisRunner` to inject `run_id`, `commis_id`, `owner_id` into the `configurable` dictionary of the fiche's `ainvoke` call.
