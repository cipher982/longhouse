"""Add unique constraint on fiche (owner_id, name)
...
def upgrade() -> None:
    ...
    if 'uq_fiche_owner_name' in existing_constraints:
        print("uq_fiche_owner_name constraint already exists - skipping")
        return

    # Find duplicate fiche IDs to delete (keep the highest ID for each owner_id, name pair)
    ...
    if duplicate_ids:
        ...
        for table in ['threads', 'courses', 'fiche_messages', 'threads', 'commis_jobs']:
            try:
                ...
                pass  # Table might not exist or have fiche_id column

        # Now delete the duplicate fiches
        conn.execute(sa.text("DELETE FROM fiches WHERE id = ANY(:ids)"), {"ids": duplicate_ids})
