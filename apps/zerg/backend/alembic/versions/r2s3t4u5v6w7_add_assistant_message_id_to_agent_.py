"""add_assistant_message_id_to_runs

Revision ID: r2s3t4u5v6w7
Revises: q1r2s3t4u5v6
Create Date: 2025-01-10 00:00:00.000000

Add assistant_message_id column to runs for durable runs message tracking.
This stores the UUID assigned to the assistant message in oikos_started,
...
def upgrade() -> None:
    """Add assistant_message_id column to runs."""
    conn = op.get_bind()
...
def downgrade() -> None:
    """Remove assistant_message_id column from runs."""
    op.drop_column('runs', 'assistant_message_id')
