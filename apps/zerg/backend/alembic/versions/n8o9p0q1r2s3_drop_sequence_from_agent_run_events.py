"""drop_sequence_from_run_events

Revision ID: n8o9p0q1r2s3
Revises: m7n8o9p0q1r2
Create Date: 2025-12-26 14:00:00.000000

Drop the sequence column from run_events table. We now use the
...
def upgrade() -> None:
    ...
    if 'run_events' not in tables:
        print("run_events table doesn't exist - skipping")
        return

    # Drop the unique constraint first
    try:
        op.drop_constraint('run_events_unique_seq', 'run_events', type_='unique')
...
def downgrade() -> None:
    ...
    op.alter_column('run_events', 'sequence', nullable=False)

    # Recreate the unique constraint
    op.create_unique_constraint('run_events_unique_seq', 'run_events', ['run_id', 'sequence'])
