...
def upgrade() -> None:
    # Add reasoning_effort column to runs for continuation inheritance
    # When a oikos run is resumed, it should use the same reasoning_effort
    ...
    result = conn.execute(
        sa.text(
            ...
            "WHERE table_name = 'runs' AND column_name = 'reasoning_effort'"
        )
    )
