...
export class TestDataGenerator {
  ...
  /**
   * Generate unique test fiche data
   */
  static generateFiche(prefix = 'Test Fiche', index?: number): Omit<TestFiche, 'id'> {
  ...
  /**
   * Generate batch of test fiches
   */
  static generateFiches(count: number, prefix = 'Batch Fiche'): Omit<TestFiche, 'id'>[] {
...
export class APITestHelper {
  constructor(private request: APIRequestContext, private commisId: string) {}

  /**
   * Create fiche with comprehensive error handling
   */
  async createFiche(ficheData: Omit<TestFiche, 'id'>): Promise<{ success: boolean; fiche?: TestFiche; error?: string }> {
    try {
      const response = await this.request.post('http://localhost:8001/api/fiches', {
        headers: {
  ...
  /**
   * Create multiple fiches concurrently
   */
  async createFichesBatch(ficheDataList: Omit<TestFiche, 'id'>[]): Promise<{
    successful: TestFiche[];
    failed: Array<{ data: any; error: string }>;
    totalTime: number;
  }> {
    const startTime = Date.now();

    const promises = ficheDataList.map(async (ficheData) => {
      const result = await this.createFiche(ficheData);
      return { ...result, originalData: ficheData };
    });
    ...
    return {
      successful: results.filter(r => r.success).map(r => r.fiche!),
  ...
  /**
   * Get all fiches with filtering
   */
  async getFiches(filters?: { name?: string; limit?: number }): Promise<TestFiche[]> {
    try {
      let url = 'http://localhost:8001/api/fiches';
...
export class DatabaseTestHelper {
  ...
  async seedTestData(scenario: 'minimal' | 'standard' | 'complex' = 'standard'): Promise<{
    ...
    const ficheData = TestDataGenerator.generateFiches(config.fiches, `Seed Fiche ${scenario}`);
...
export class TestHelperFactory {
  static createAPIHelper(request: APIRequestContext, commisId: string): APITestHelper {
    return new APITestHelper(request, commisId);
  }
  ...
  static createAllHelpers(page: Page, request: APIRequestContext, commisId: string) {
    const apiHelper = this.createAPIHelper(request, commisId);
