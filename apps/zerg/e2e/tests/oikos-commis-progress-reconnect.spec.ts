/**
 * Oikos commis progress reconnection regression test.
 *
 ...
 * - clear the panel after oikos_complete (no commis pending)
 */
...
test('Oikos commis progress reconciles orphan commis and clears on complete', async ({ page }) => {
  ...
  await page.waitForFunction(() => (window as any).__oikos?.eventBus != null, null, { timeout: 15_000 });
  ...
  await page.evaluate(() => {
    ...
    bus.emit('oikos:commis_complete', {
      jobId: 1,
      commisId,
      status: 'success',
      durationMs: 123,
  ...
  await expect(progress.locator('.oikos-active-count')).toHaveCount(0, { timeout: 5_000 });

  // And the whole panel should clear shortly after oikos_complete (2s debounce).
