...
/**
 * E2E Tests for Oikos Tool Visibility
 *
 * Tests the inline display of oikos tool calls in the Oikos chat UI.
...
async function emitToolStarted(
  ...
  await page.evaluate(
    ({ toolName, toolCallId, runId }) => {
      (window as any).__oikos?.eventBus?.emit('oikos:tool_started', {
        runId,
        toolName,
        toolCallId,
...
async function emitToolCompleted(
  ...
  await page.evaluate(
    ({ toolName, toolCallId, runId, durationMs }) => {
      (window as any).__oikos?.eventBus?.emit('oikos:tool_completed', {
        runId,
        toolName,
        toolCallId,
        durationMs,
...
async function emitToolFailed(
  ...
  await page.evaluate(
    ({ toolName, toolCallId, runId, error }) => {
      (window as any).__oikos?.eventBus?.emit('oikos:tool_failed', {
        runId,
        toolName,
        toolCallId,
        durationMs: 500,
...
async function emitOikosStarted(page: Page, runId: number): Promise<void> {
  await page.evaluate(
    ({ runId }) => {
...
test.describe('Oikos Tool Visibility', () => {
  // NOTE: These deterministic tests use window.__oikos.eventBus for event injection,
  ...
  test.skip(true, 'Skipped: window.__oikos only exposed in DEV mode, E2E uses prod build');

  test('displays tool card when oikos calls a tool', async ({ page }) => {
    await navigateToChatPage(page);

    const runId = 1;
    await emitOikosStarted(page, runId);
...
test.describe('Oikos Tool Visibility - Edge Cases', () => {
  // Same as above - event injection only works in DEV mode
  test.skip(true, 'Skipped: window.__oikos only exposed in DEV mode');

  test('handles missing runId gracefully', async ({ page }) => {
    await navigateToChatPage(page);

    // Emit tool event without starting oikos (no runId)
    await page.evaluate(() => {
      (window as any).__oikos?.eventBus?.emit('oikos:tool_started', {
