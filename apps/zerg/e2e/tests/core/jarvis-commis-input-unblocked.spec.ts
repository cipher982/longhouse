/**
 * Jarvis non-blocking commis UX regression test.
 *
 * Ensures chat input re-enables after concierge_complete even if
 * a background commis is still running (simulated by a hanging /chat fetch).
 */

import { test, expect } from '../fixtures';
import { waitForReadyFlag, waitForEventBusAvailable } from '../helpers/ready-signals';

test('chat input unblocks after concierge_complete while commis runs', async ({ page }) => {
  ...
  // Pre-fill follow-up while the first course is still "in-flight".
  await input.fill('Follow up while commis runs');

  // Simulate background commis spawned + concierge completes immediately.
  await page.evaluate(() => {
  ...
  // Input should re-enable even though commis is still "running".
