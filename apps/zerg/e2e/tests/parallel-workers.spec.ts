...
test.describe('Parallel Workers Barrier', () => {
  test.beforeEach(async ({ request }) => {
    await resetDatabase(request);
  });

  test('spawns multiple workers in a single interrupt and resumes after all complete', async ({ request }) => {
    ...
    await expect
      .poll(async () => {
        const runsRes = await request.get('/api/jarvis/runs?limit=50');
    ...
    if (!runId) {
      throw new Error('Failed to locate parallel-worker run');
    }
    ...
    await expect
      .poll(async () => {
        ...
        const waitingCount = events.filter((e) => e.event_type === 'supervisor_waiting').length;
