 ...
 */
export async function editAutomationViaUI(
  page: Page,
  automationId: string,
  data: {
    name?: string;
  ...
  testLog.info(`Automation ${automationId} edited via UI`);
}

/**
 * Delete an automation via UI with confirmation.
 */
export async function deleteAutomationViaUI(page: Page, automationId: string, confirm: boolean = true): Promise<void> {
  ...
  if (confirm) {
    ...
    testLog.info(`Automation ${automationId} deleted via UI`);
  } else {
    await expect(page.locator(`tr[data-automation-id="${automationId}"]`)).toHaveCount(1);
    testLog.info(`Automation ${automationId} deletion cancelled`);
  }
}
