...
test.describe('Core User Journey - Scripted LLM', () => {
  ...
  test('supervisor direct response flow with gpt-scripted model', async ({ page, request }) => {
    ...
    // Step 1: Navigate to chat (needed to establish auth context)
    await navigateToChatPage(page);
    console.log('[Core Journey] Chat page loaded');

    // Step 2: Send a simple message that uses the "generic_supervisor" scenario
    ...
    // Poll until we have events (events are persisted async)
    await expect.poll(
      async () => {
        allEvents = await getRunEvents(request, runId!);
        return allEvents.total;
      },
