...
test.describe('Fiche scheduling UI', () => {
  ...
  async function createAndOpenConfig(page) {
    ...
    const row = page.locator(`tr[data-fiche-id="${id}"]`);
    await expect(row).toBeVisible({ timeout: 10000 });

    await page.locator(`[data-testid="debug-fiche-${id}"]`).click();
    await page.waitForSelector('[data-testid="fiche-debug-modal"]', { state: 'visible' });
    return id;
  }

  test('Set cron schedule on fiche', async ({ page }) => {
    ...
    if (await scheduledStatus.count() === 0) {
      // Fallback: just check that the fiche row still exists and modal closed
      await expect(page.locator('tr[data-fiche-id]')).toHaveCount(1, { timeout: 5000 });
