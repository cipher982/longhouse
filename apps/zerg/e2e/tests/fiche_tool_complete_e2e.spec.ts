...
/**
 * Complete E2E test for Fiche + Tool Workflow
 *
 * This test replicates the exact browser workflow for connecting fiches to tools
 * and executing workflows, covering the complete user journey:
 *
 * 1. Dashboard - Create Fiche
 * 2. Canvas - Drag Fiche from Shelf
 * 3. Canvas - Drag URL Tool from Palette
 * 4. Canvas - Connect Nodes (trigger → fiche → tool)
 * 5. Execution - Run Workflow and verify HTTP request execution
 */

test.describe('Fiche + Tool Complete E2E Workflow', () => {
  ...
  test.beforeEach(async ({ page, request }) => {
    ...
    // Verify clean state - should have no fiches in fresh in-memory DB
    await page.locator('.header-nav').click();
    await page.waitForTimeout(1000);
    const existingFiches = await page.locator('table tbody tr').count();
    console.log(`🧹 Clean in-memory DB state: ${existingFiches} existing fiches`);

    // Should be 0 fiches in fresh in-memory database
    if (existingFiches > 0) {
      console.warn(`⚠️ Expected 0 fiches in fresh test DB, found ${existingFiches}`);
    }
  });

  test('Complete fiche-tool workflow: create → drag → connect → execute', async ({ page }) => {
    // ========================================================================
    // STEP 1: Dashboard - Create Fiche
    // ========================================================================
    ...
    // Create just ONE fiche to avoid confusion
    const createFicheBtn = page.locator('button:has-text("Create Fiche")');
    await createFicheBtn.click();
    await page.waitForTimeout(1000);

    // Verify fiche was created (flexible count for testing)
    ...
    console.log(`✅ Created ${ficheCount} fiche(s) total`);
    expect(ficheCount).toBeGreaterThanOrEqual(1); // At least 1 fiche exists
    console.log('✅ Step 1 Complete: Created one fiche');

    // ========================================================================
    // STEP 2: Canvas - Switch to Canvas and Load Fiche Shelf
    // ========================================================================
    console.log('🎨 Step 2: Switching to canvas and loading fiche shelf...');
    ...
    // Wait for fiche shelf to load with our created fiche
    ...
    console.log('✅ Step 2 Complete: Canvas loaded with fiche shelf');

    // ========================================================================
    // STEP 3: Canvas - Drag Fiche from Shelf onto Canvas
    // ========================================================================
    console.log('🖱️ Step 3: Dragging fiche from shelf to canvas...');

    const fichePill = page.locator('#fiche-shelf .fiche-pill').first();
    const canvasArea = page.locator('#canvas-container canvas');

    // Drag fiche to canvas at position (200, 150)
    await fichePill.dragTo(canvasArea, {
      targetPosition: { x: 200, y: 150 }
    });
    await page.waitForTimeout(1000);

    // Verify fiche node appears on canvas - try multiple selectors
    let ficheNodes = page.locator('.canvas-node, .generic-node, [data-node-type="fiche"], .node, .workflow-node');

    // Wait and see if any nodes appeared
    await page.waitForTimeout(2000);
    const nodeCount = await ficheNodes.count();
    ...
    console.log('✅ Step 3 Complete: Fiche dragged to canvas');
    ...
    // Success criteria for fiche-tool integration
    ...
    console.log('🎉 Fiche-Tool Complete E2E Test SUCCESS!');
    console.log('✅ Workflow created, configured, and executed successfully');
    console.log('✅ Fiche-tool integration validated end-to-end');
  });
  ...
  test('Verify fiche shelf loading and drag functionality', async ({ page }) => {
    console.log('🧪 Testing fiche shelf functionality...');

    // First create an fiche
    await page.locator('.header-nav').click();
    await page.waitForTimeout(1000);
    ...
    // Verify fiche appears in shelf
    const shelfPills = page.locator('#fiche-shelf .fiche-pill');
    await expect(shelfPills.first()).toBeVisible({ timeout: 10000 });

    // Test fiche pill drag functionality
    const fichePill = page.locator('#fiche-shelf .fiche-pill').first();
    await expect(fichePill).toBeVisible();

    // Verify fiche pill has drag attributes
    const isDraggable = await fichePill.getAttribute('draggable');
    expect(isDraggable).toBe('true');

    console.log('✅ Fiche shelf functionality test complete');
  });
});
