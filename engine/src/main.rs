...
enum DeviceCommands {
    ...
    /// Restart an existing configured Machine Agent service when native repair can do so safely
    Repair {
        /// Emit the native repair execution result as JSON
        #[arg(long)]
        json: bool,

        /// Plan the native repair action without restarting the service
        #[arg(long)]
        dry_run: bool,
...
enum CodexBridgeCommands {
    ...
    Run {
        ...
        /// Launch-scoped execution identity forwarded by `codex-bridge start`.
        #[arg(long, hide = true)]
        run_id: Option<String>,
...
mod tests {
    ...
    #[test]
    fn codex_bridge_start_detached_ui_launch_mode_is_explicit() {
        let cli = Cli::try_parse_from([
            "longhouse-engine",
            "codex-bridge",
            "start",
