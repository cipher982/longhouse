...
mod tests {
    ...
    #[test]
    fn test_archive_replay_uses_idempotent_ingest_without_source_line_preflight() {
        let rt = tokio::runtime::Runtime::new().unwrap();
        let (_tmp, conn) = make_db();
        let dir = tempfile::tempdir().unwrap();
        let image_data = "A".repeat(4096);
    ...
    #[test]
    fn test_archive_ship_uses_idempotent_ingest_without_source_line_preflight() {
        let rt = tokio::runtime::Runtime::new().unwrap();
        let (_tmp, conn) = make_db();
        let dir = tempfile::tempdir().unwrap();
        let image_data = "A".repeat(4096);
    ...
    #[test]
    fn test_archive_replay_skips_source_line_preflight() {
        let rt = tokio::runtime::Runtime::new().unwrap();
        let (_tmp, conn) = make_db();
        let dir = tempfile::tempdir().unwrap();
        let image_data = "A".repeat(4096);
        ...
        let (url, captured, handle) = spawn_http_sequence_server_with_headers(&[
        ...
        assert!(requests[0].0.starts_with("POST /api/agents/media/claims "));
        assert!(requests[1].0.starts_with("POST /api/agents/ingest "));
        assert_eq!(replay.resolved, 1);
        assert_eq!(replay.failed, 0);
        assert_eq!(replay.events_shipped, 1);
        assert_eq!(spool.pending_count().unwrap(), 0);
