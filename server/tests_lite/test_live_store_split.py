...
def test_live_launch_readiness_reap_keeps_pending_remote_launch_outbox(tmp_path):
    now = datetime.now(timezone.utc)
    live_engine = make_live_engine(f"sqlite:///{tmp_path}/live-remote-launch-reap.db")
    initialize_live_database(live_engine)
    LiveSession = sessionmaker(bind=live_engine)
    intent_session_id = uuid4()
    outcome_session_id = uuid4()
    ordinary_expired_session_id = uuid4()

    def launch_payload(session_id):
        return {
    ...
    try:
        with LiveSession() as live_db:
            for session_id in (intent_session_id, outcome_session_id, ordinary_expired_session_id):
                upsert_live_launch_readiness(
                    live_db,
                    session_id=session_id,
                    owner_id=42,
            ...
            enqueue_remote_launch_outbox(live_db, launch=launch_payload(intent_session_id))
            enqueue_remote_launch_outcome_outbox(
                live_db,
                launch=launch_payload(outcome_session_id),
        ...
        assert removed == 1
        with LiveSession() as live_db:
            assert live_db.get(LiveLaunchReadiness, str(intent_session_id)) is not None
            assert live_db.get(LiveLaunchReadiness, str(outcome_session_id)) is not None
            assert live_db.get(LiveLaunchReadiness, str(ordinary_expired_session_id)) is None
    finally:
        live_engine.dispose()


def test_remote_launch_outbox_out_of_order_drain_keeps_dispatched_state(tmp_path):
    now = datetime.now(timezone.utc)
    live_engine = make_live_engine(f"sqlite:///{tmp_path}/live-remote-launch-order.db")
    initialize_live_database(live_engine)
    LiveSession = sessionmaker(bind=live_engine)
    archive_engine = make_engine(f"sqlite:///{tmp_path}/archive-remote-launch-order.db")
    Base.metadata.create_all(bind=archive_engine)
    ArchiveSession = sessionmaker(bind=archive_engine)
    session_id = uuid4()
    launch = {
    ...
    try:
        with LiveSession() as live_db:
            ...
            enqueue_remote_launch_outbox(live_db, launch=launch)
            enqueue_remote_launch_outcome_outbox(
                live_db,
                launch=launch,
        ...
        with ArchiveSession() as archive_db:
            attempt = archive_db.query(SessionLaunchAttempt).one()
            assert attempt.state == "dispatched"
            assert attempt.error_message == "timed out"
        with LiveSession() as live_db:
...
def test_remote_launch_outcome_drain_does_not_regress_adopted_readiness(tmp_path):
    now = datetime.now(timezone.utc)
    live_engine = make_live_engine(f"sqlite:///{tmp_path}/live-remote-launch-outcome-order.db")
    initialize_live_database(live_engine)
    LiveSession = sessionmaker(bind=live_engine)
    archive_engine = make_engine(f"sqlite:///{tmp_path}/archive-remote-launch-outcome-order.db")
    Base.metadata.create_all(bind=archive_engine)
    ArchiveSession = sessionmaker(bind=archive_engine)
    session_id = uuid4()
    launch = {
    ...
    try:
        with LiveSession() as live_db:
            ...
            enqueue_remote_launch_outcome_outbox(
                live_db,
                launch=launch,
                outcome={"state": "adopted", "external_name": "cinder"},
            )
            enqueue_remote_launch_outcome_outbox(
                live_db,
                launch=launch,
                outcome={"state": "dispatched", "error_message": "late timeout"},
            )
            adopted = (
                live_db.query(LiveArchiveOutbox)
                .filter(LiveArchiveOutbox.idempotency_key.like(f"remote_launch_outcome.v1:{session_id}:adopted%"))
                .one()
            )
            dispatched = (
                live_db.query(LiveArchiveOutbox)
                .filter(LiveArchiveOutbox.idempotency_key.like(f"remote_launch_outcome.v1:{session_id}:dispatched%"))
                .one()
            )
            adopted.created_at = now - timedelta(seconds=2)
            dispatched.created_at = now - timedelta(seconds=1)
            live_db.commit()
