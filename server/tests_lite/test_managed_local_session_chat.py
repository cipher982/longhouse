    ...
    with session_local() as db:
        ...
        def fake_get_turn_snapshot(*, db_bind, session_id, request_id):
            ...
            return SimpleNamespace(
                ...
                durable_user_event_id=101,
                durable_assistant_event_id=102,
                durable_at=datetime.now(timezone.utc),
                error_code=None,
            )

        def fake_fetch_events_between_ids(*, db_bind, session_id, start_event_id, end_event_id):
            assert session_id == source_session.id
            assert start_event_id == 101
            assert end_event_id == 102
            return [
                SimpleNamespace(
                    id=101,
                    role="user",
        ...
        monkeypatch.setattr("zerg.routers.session_chat._fetch_managed_local_events_between_ids", fake_fetch_events_between_ids)
        monkeypatch.setattr(session_chat, "MANAGED_LOCAL_POST_TERMINAL_SYNC_GRACE_SECS", 0.01)
...
def test_chat_with_session_waits_briefly_for_terminal_after_durable_events(monkeypatch, tmp_path):
    session_local = _make_db(tmp_path)
    ...
    with session_local() as db:
        ...
        async def fake_wait_for_events(**_kwargs):
            ...
            assistant_event = session_chat.AgentEvent(
                session_id=source_session.id,
                role="assistant",
                content_text="Durable reply before terminal hook settles",
                tool_name=None,
                tool_call_id=None,
                timestamp=datetime.now(timezone.utc),
            )
        ...
        monkeypatch.setattr(session_chat, "MANAGED_LOCAL_POST_DURABLE_TERMINAL_GRACE_SECS", 0.05)
        ...
        try:
            ...
            assert response.status_code == 200, response.text
            body = response.text
            assert "Durable reply before terminal hook settles" in body
            assert '"sync_status": "complete"' in body
            assert '"control_status": "completed"' in body
            assert '"persisted_events": 2' in body
            ...
            assert len(turn_rows) == 1
            assert turn_rows[0].terminal_phase == "idle"
            assert turn_rows[0].terminal_runtime_event_id == 104
            assert turn_rows[0].durable_at is not None
            assert turn_rows[0].durable_user_event_id is not None
            assert turn_rows[0].durable_assistant_event_id is not None
        finally:
...
def test_chat_with_session_ignores_terminal_waiter_error_after_durable_events(monkeypatch, tmp_path):
    session_local = _make_db(tmp_path)
    ...
    with session_local() as db:
        ...
        async def fake_wait_for_terminal(**_kwargs):
            await asyncio.sleep(0.02)
            raise RuntimeError("terminal poll blew up")
        ...
        async def fake_wait_for_events(**_kwargs):
            ...
            assistant_event = session_chat.AgentEvent(
                session_id=source_session.id,
                role="assistant",
                content_text="Durable reply survives terminal waiter error",
                tool_name=None,
                tool_call_id=None,
                timestamp=datetime.now(timezone.utc),
            )
        ...
        try:
            ...
            assert response.status_code == 200, response.text
            body = response.text
            assert "Durable reply survives terminal waiter error" in body
            assert '"sync_status": "complete"' in body
            assert '"control_status": "completed"' in body
            assert '"persisted_events": 2' in body
            ...
            assert len(turn_rows) == 1
            assert turn_rows[0].terminal_phase is None
            assert turn_rows[0].terminal_runtime_event_id is None
            assert turn_rows[0].durable_at is not None
            assert turn_rows[0].durable_user_event_id is not None
            assert turn_rows[0].durable_assistant_event_id is not None
        finally:
...
def test_chat_with_session_falls_back_when_ledger_snapshot_read_times_out(monkeypatch, tmp_path):
    session_local = _make_db(tmp_path)
    ...
    with session_local() as db:
        ...
        async def fake_wait_for_events(**_kwargs):
            return [
                ...
                SimpleNamespace(
                    id=102,
                    role="assistant",
                    content_text="Direct evidence reply",
                    tool_name=None,
                    tool_call_id=None,
                ),
            ]
        ...
        try:
            ...
            assert response.status_code == 200, response.text
            body = response.text
            assert "Direct evidence reply" in body
            assert '"sync_status": "complete"' in body
            assert '"control_status": "completed"' in body
            assert '"persisted_events": 2' in body
        finally:
