...
def test_antigravity_hook_inbox_passes_contract_send_through(db):
    ...
    # The contract implements send, so a connection carrying send=1 keeps it.
    # The clamp still holds down every operation the contract lacks, and the
    # honesty gate for send lives past this row: the served grant additionally
    # requires a control fact whose granted_operations names it.
    assert caps.can_send_input is True
...
def test_known_provider_control_plane_clamps_capability_bits_to_contract(db):
    ...
    # Send is implemented in the contract and passes the clamp; what the clamp
    # holds down is everything the contract still lacks.
    assert caps.can_send_input is True
...
def test_live_session_capabilities_match_provider_action_coverage(db):
    ...
    for contract in all_managed_provider_contracts():
        ...
        # Terminate is provider-contract lifecycle control, not a provider
        # action-coverage row, so keep this affordance tied to the contract.
        assert caps.can_terminate is bool(contract.terminate)
