...
def _control_fact(
    *,
    ...
    value = {
        ...
        "state": "attached",
        "lease_ttl_ms": 60_000,
        "source": f"{provider}_helm_scan" if provider == "omp" else "codex_bridge_scan",
...
def test_omp_control_identity_accepts_a_safe_generation_only_replacement(tmp_path):
    ...
    with engine.begin() as connection:
        ...
        connection.execute(
            LiveSessionConnection.__table__.insert().values(
                run_id=run_id,
                control_plane=contract_for_provider("omp").control_plane,
                acquisition_kind="spawned_control",
                state="attached",
                device_id=DEVICE_ID,
                acquired_at=now,
        ...
        assert _bind_control_evidence_identities(connection, [first], device_id=DEVICE_ID)["bound"] == 1
        reduce_fact_batch(connection, [first], received_at=now)
        second = _control_fact(
            session_id=session_id,
            run_id=run_id,
            connection_id=adapter_connection_id,
        ...
        result = _bind_control_evidence_identities(connection, [second], device_id=DEVICE_ID)
