"""Voice implementation of the SurfaceAdapter contract."""
...
class VoiceSurfaceAdapter:
    """Adapter for turn-based voice ingress."""

    surface_id = "voice"
    mode = "inline"

    def __init__(self, *, owner_id: int, conversation_id: str = "voice:default") -> None:
    ...
    async def normalize_inbound(self, raw_input: Any) -> SurfaceInboundEvent | None:
        ...
        dedupe_key = f"voice:{owner_hint}:{message_id}"
    ...
    def build_run_kwargs(self, event: SurfaceInboundEvent) -> dict[str, Any]:
        timeout_raw = (event.raw or {}).get("timeout", 60)
        try:
