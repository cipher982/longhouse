...
def create_server(api_url: str, api_token: str | None = None) -> FastMCP:
    ...
    async def poke(
        ...
        """Send a poke to another session — tap them on the shoulder.

        A poke is a lightweight pointer, not a message thread. Include a short
        note explaining what to look at, and optionally reference a specific
        event ID from your session. The recipient reads your session tail for
        full context.

        Args:
            from_session_id: Your session ID (the sender).
            to_session_id: Target session ID (from check_wall results).
            note: Short message — what should they look at and why.
            source_event_id: Optional event ID in your session to reference.
        """
        if not _UUID_RE.match(from_session_id) or not _UUID_RE.match(to_session_id):
            return json.dumps({"error": "Invalid session_id format — expected UUID"})
        ...
        try:
            resp = await client.post("/api/agents/pokes", json=body)
            if resp.status_code not in (200, 201):
                return json.dumps({"error": f"API returned {resp.status_code}", "detail": resp.text[:500]})
            return resp.text
        except Exception as exc:
    ...
    # ------------------------------------------------------------------
    @server.tool()
    async def check_pokes(
        session_id: str,
        unread_only: bool = True,
    ) -> str:
        """Check for pokes directed at your session.

        Returns pokes from other sessions that want your attention. Reading
        pokes marks them as read. Follow up by reading the sender's session
        tail for full context.

        Args:
            session_id: Your session ID to check pokes for.
            unread_only: Only return unread pokes (default True).
        """
        if not _UUID_RE.match(session_id):
            return json.dumps({"error": "Invalid session_id format — expected UUID"})
