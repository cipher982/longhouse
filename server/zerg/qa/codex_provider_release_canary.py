...
def _bridge_state_root(isolation_root: Path) -> Path:
    # `codex-bridge --isolation-root` resolves to this exact path. Keep the
    # sender, stop path, and the disposable Machine Agent on one durable state
    # root so the bridge lease and terminal Resume contract cannot diverge.
