...
def load_request(
    ...
    *,
    ...
    if not IDENTITY.fullmatch(payload["expected_provider_build_identity"]):
        raise RequestError("expected_provider_build_identity must be sha256:<64 lowercase hex>")
    if payload["expected_provider_build_granularity"] not in {"full_installed_tree", "single_asset"}:
        raise RequestError("expected_provider_build_granularity must be full_installed_tree or single_asset")
