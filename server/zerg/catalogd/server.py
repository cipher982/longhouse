...
class CatalogDaemon:
    ...
    async def _rename_machine_enrollment(self, request: CatalogRpcRequest) -> CatalogRpcResponse:
        ...
        if not isinstance(device_id, str) or not 1 <= len(device_id) <= 255:
            return self._error(request, "invalid_request", "device_id must contain 1 to 255 characters")
        if not isinstance(machine_name, str) or not 1 <= len(machine_name) <= 255 or machine_name != machine_name.strip():
            return self._error(request, "invalid_request", "machine_name must be 1 to 255 trimmed characters")
        assert self._store is not None
        result = await self._run_store(
            self._store.rename_machine,
            owner_id=owner_id,
            device_id=device_id,
    ...
    async def _read_storage_session_render_manifest(self, request: CatalogRpcRequest) -> CatalogRpcResponse:
        ...
        if after_order_key is not None and before_order_key is not None:
            return self._error(request, "invalid_request", "only one render cursor direction may be set")
...
def _validate_raw_object_commit(params: dict) -> None:
    ...
    # Raw record bytes are capped at 32 MiB. The self-describing object adds a
    # bounded header plus one 12-byte position/length tuple per record.
    for field, maximum in (("uncompressed_size", 33 * 1024 * 1024), ("compressed_size", 40 * 1024 * 1024)):
        value = params[field]
        if type(value) is not int or not 0 <= value <= maximum:
            raise ValueError(f"{field} exceeds its storage-v2 bound")
    if params["provenance_kind"] not in {
        "native",
        "legacy_source_lines",
        "legacy_fallback",
