...
class TestPathValidation:
    ...
    def test_path_traversal_with_dotdot(self, client: TestClient) -> None:
        ...
        response = client.get(
            "/api/skills",
            params={"workspace_path": "/var/longhouse/workspaces/../../../etc"},
        )
        assert response.status_code == 400
    ...
    def test_valid_workspace_path(self, client: TestClient) -> None:
        ...
        response = client.get(
            "/api/skills",
            params={"workspace_path": str(Path.home() / ".longhouse" / "workspaces" / "test")},
        )
...
class TestSkillsAPI:
    ...
    def test_list_skills_with_workspace(self, client: TestClient, skill_workspace: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # Set LONGHOUSE_WORKSPACE_PATH to parent of test workspace to allow access
        monkeypatch.setenv("LONGHOUSE_WORKSPACE_PATH", str(skill_workspace.parent))
