...
# Linux: systemd user service (`desktop`) or system service (`server`)
...
RUNNER_AVAILABILITY_POLICY="${RUNNER_AVAILABILITY_POLICY:-}"
...
if [ -z "$RUNNER_AVAILABILITY_POLICY" ]; then
  if [ "$RUNNER_INSTALL_MODE" = "server" ]; then
    RUNNER_AVAILABILITY_POLICY="always_on"
  else
    RUNNER_AVAILABILITY_POLICY="on_demand"
  fi
fi

case "$RUNNER_AVAILABILITY_POLICY" in
  always_on|on_demand|ephemeral) ;;
  *)
    echo "Error: RUNNER_AVAILABILITY_POLICY must be 'always_on', 'on_demand', or 'ephemeral'" >&2
    exit 1
    ;;
esac
...
RESPONSE=$(curl -sf -X POST "$REGISTER_URL" \
  ...
  -d "{\"enroll_token\": \"$ENROLL_TOKEN\", \"name\": \"$RUNNER_NAME\", \"availability_policy\": \"$RUNNER_AVAILABILITY_POLICY\", \"capabilities\": $REQUESTED_CAPS_JSON}" 2>&1) || {
