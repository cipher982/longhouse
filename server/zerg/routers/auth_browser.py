...
_pending_google_states: dict[str, float] = {}


@router.get("/google/redirect")
async def google_redirect(request: Request):
    """Server-side OAuth redirect for native apps (ASWebAuthenticationSession)."""
    settings = get_settings()
    if not settings.google_client_id or not settings.google_client_secret:
        raise HTTPException(status_code=500, detail="Google OAuth not configured")

    state = secrets.token_urlsafe(32)
    _pending_google_states[state] = time.time()

    cutoff = time.time() - 600
    stale = [k for k, v in _pending_google_states.items() if v < cutoff]
    for k in stale:
        _pending_google_states.pop(k, None)

    callback_url = f"{settings.app_public_url.rstrip('/')}/api/auth/google/callback"
...
async def google_callback(
    ...
    """Exchange Google auth code, then redirect to longhouse:// with tokens."""
    if error:
        raise HTTPException(status_code=400, detail=f"Google auth error: {error}")

    if not state or state not in _pending_google_states:
        raise HTTPException(status_code=400, detail="Invalid or expired state")
    _pending_google_states.pop(state, None)
    ...
    if settings.single_tenant and not settings.testing:
        ...
        if not is_owner_email(email):
            raise HTTPException(status_code=403, detail="Sign-in restricted to instance owner")
    ...
    token_out = await _issue_session(response, db, user)

    raw_rt = None
    for header_name, header_value in response.headers.multi_items():
        if header_name.lower() == "set-cookie" and "longhouse_refresh=" in header_value:
            raw_rt = header_value.split("longhouse_refresh=")[1].split(";")[0]

    import urllib.parse

    params = {"at": token_out.access_token}
    if raw_rt:
        params["rt"] = raw_rt
    from starlette.responses import RedirectResponse as StRedirect

    return StRedirect(f"longhouse://auth-callback?{urllib.parse.urlencode(params)}", status_code=302)
