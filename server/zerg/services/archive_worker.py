...
async def _run_worker(
    *,
    ...
    # The child uses the archive serializer for its maintenance units, but it
    # must not clear or publish the Runtime Host's cross-process writer lease.
    configure_write_serializer(observe_api_activity=False)
    live_session_factory = get_live_session_factory()
        ...
        try:
            while not stop_requested:
                ...
                if result.get("processed") and not result.get("failed"):
                    await asyncio.sleep(_busy_yield_seconds())
                    continue
