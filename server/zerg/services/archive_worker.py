...
def _acknowledge_outbox_row(live_session_factory, *, row_id: int, error: str | None) -> bool:
...
def drain_once(*, limit: int | None = None) -> dict[str, int]:
    ...
    from zerg.services.live_archive_outbox import _drain_heartbeat_stamp
    ...
    del limit  # Phase 2 owns one heartbeat row per cold transaction.
    try:
        with archive_session_factory() as archive_db:
            _drain_heartbeat_stamp(SimpleNamespace(payload_json=envelope["payload_json"]), archive_db)
            archive_db.commit()
    except Exception as exc:
        error = f"{type(exc).__name__}: {exc}"
        _acknowledge_outbox_row(live_session_factory, row_id=int(envelope["id"]), error=error)
        return {"processed": 1, "drained": 0, "failed": 1}

    drained = _acknowledge_outbox_row(live_session_factory, row_id=int(envelope["id"]), error=None)
