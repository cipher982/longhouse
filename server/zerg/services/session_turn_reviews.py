...
from zerg.services.turn_review_analysis import _EXPECTED_CONTINUE_OUTCOME
from zerg.services.turn_review_analysis import _EXPECTED_IGNORE_OUTCOME
from zerg.services.turn_review_analysis import _classify_alignment
from zerg.services.turn_review_analysis import _expected_outcome
...
from zerg.services.turn_review_notifications import _ATTENTION_EXECUTION_STATES
...
async def maybe_process_session_turn_loop(
    *,
    ...
    if session is not None and is_managed_local_session:
        attached = run_best_effort_managed_local_turn_write(
            db_bind=db.get_bind(),
            label="managed-local-turn-review-attach",
            fn=lambda turn_db: attach_review_to_managed_local_turn(
                turn_db,
                session_id=session.id,
