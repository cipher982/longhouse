...
def _validate_managed_local_launch_response_contract(
    *,
    ...
    if transport == ManagedSessionTransport.CURSOR_HELM:
        if "longhouse cursor --resume-session" not in attach_command or sid not in attach_command:
            raise RuntimeError("Cursor Helm managed local launch response is missing native resume command")
        return

    if transport == ManagedSessionTransport.PI_HELM_CHANNEL:
        if not response.provider_session_id:
            raise RuntimeError("Pi Helm launch response is missing native session identity")
        if "longhouse pi --resume-session" not in attach_command or sid not in attach_command:
...
def _load_session_for_continuation(db: Session, session_id: str, *, owner_id: int):
    ...
    if db is None:
        ...
        # A catalog that could not answer is not the same as a session that does
        # not exist; the 404 below is only for the latter.
        try:
...
def _assert_live_session_action_available(
    ...
    *,
    ...
    if getattr(source_session, "catalog_facts", None) is not None:
