"""Immutable archive writes for new ingest payloads."""
...
@dataclass(frozen=True)
class PreparedArchivePrimary:
    chunks: tuple[ArchiveChunkRef, ...] = ()
    records_written: int = 0
    error: str | None = None


def write_ingest_archive(
    db: Session,
    *,
    data: SessionIngest,
    result: IngestResult,
    ...
    if prepared.error:
        return ArchivePrimaryResult(error=prepared.error)
    if not prepared.chunks:
        return ArchivePrimaryResult(records_written=prepared.records_written)

    try:
        insert_archive_chunk_manifests(db, prepared.chunks)
        return ArchivePrimaryResult(
            chunks_written=len(prepared.chunks),
            records_written=prepared.records_written,
        )
    except Exception as exc:
        db.rollback()
        logger.warning("Archive manifest write failed for session %s: %s", result.session_id, exc, exc_info=True)
        return ArchivePrimaryResult(error=type(exc).__name__)


def prepare_ingest_archive(
    *,
    data: SessionIngest,
    result: IngestResult,
    settings: Settings | None = None,
    ...
    try:
        ...
        if source_line_records:
            chunks.extend(
                store.write_record_chunks(
                    source_line_records,
                    target_uncompressed_bytes=max(1, int(settings.archive_primary_chunk_target_bytes)),
                )
            )
        ...
        logger.warning("Archive chunk write failed for session %s: %s", result.session_id, exc, exc_info=True)
        return PreparedArchivePrimary(error=type(exc).__name__)
    ...
    *,
    ...
    for chunk in chunks:
        try:
            ...
            logger.warning(
                "Skipping unreadable archive chunk while filtering archive ingest overlap: %s: %s",
                chunk.relative_path,
                exc,
                exc_info=True,
            )
