...
class SessionLivePreview(AgentsBase):
    ...
    preview_role = Column(String(32), nullable=False, server_default=text("'assistant'"))
    tool_name = Column(String(128), nullable=True)
    tool_input_json = Column(Text, nullable=True)
    tool_output_text = Column(Text, nullable=True)
    tool_call_id = Column(String(255), nullable=True)
    tool_call_state = Column(String(32), nullable=True)
    provisional_cursor = Column(String(512), nullable=True)
    provisional_complete = Column(Integer, nullable=False, server_default=text("0"))
...
class SessionInput(AgentsBase):
    ...
    Holds queued drafts that drain at the next safe turn boundary and records the effective delivery
